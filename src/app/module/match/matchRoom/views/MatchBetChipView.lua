...
local MatchBetChipView = class("MatchBetChipView")
...
MatchBetChipView.MOVE_FROM_SEAT_DURATION = 0.4
MatchBetChipView.MOVE_TO_POT_DURATION = 0.5
local MOVE_DELAY_DURATION = 0.075

local GAP_WITH_CHIPS = 4

function MatchBetChipView:ctor(parent, manager, seatId)
    self.parent_ = parent
...
function MatchBetChipView:rotate(positionId)
    if self.betChipData_ then
        for i, chipData in ipairs(self.betChipData_) do
            chipData:getSprite():pos(BP[positionId].x - 38, BP[positionId].y + (i - 1) * GAP_WITH_CHIPS)
        end
        return self
    end
end
...
-- 创建筹码堆
function MatchBetChipView:createChipStack()
    if self.betChipData_ then
        local positionId = self.manager_.seatManager:getSeatPositionId(self.seatId_)
        for i, chipData in ipairs(self.betChipData_) do
            local sp = chipData:getSprite()
            if sp:getParent() then
                sp:pos(BP[positionId].x - 38, BP[positionId].y + (i - 1) * GAP_WITH_CHIPS):opacity(255)
            else
                sp:pos(BP[positionId].x - 38, BP[positionId].y + (i - 1) * GAP_WITH_CHIPS):opacity(255):addTo(self.parent_)
            end
        end
...
-- 重置筹码堆
function MatchBetChipView:resetChipStack(betChips)
...
-- 获取某一轮当前下注筹码
function MatchBetChipView:getBetTotalChips()
    return self.betTotalChips_ or 0
end

-- 从座位飞出
function MatchBetChipView:moveFromSeat(betNeedChips, betTotalChips)
    dump(betNeedChips,"moveFromSeat")
    dump(betTotalChips,"betTotalChips")
        ...
        for i, chipData in ipairs(self.movingChipData_) do
            ...
            if i < #self.movingChipData_ then
                transition.execute(
                    sp, 
                    cc.MoveTo:create(
                        MatchBetChipView.MOVE_FROM_SEAT_DURATION, 
                        cc.p(BP[positionId].x - 38, BP[positionId].y + (i + lastBetChipNum - 1) * GAP_WITH_CHIPS)
                    ), 
...
function MatchBetChipView:moveFromSeatComplete_()
    -- 有上一次的下注筹码数据，则回收
    if self.lastBetChipData_ then
        if self.lastBetChipData_ == self.potChipData_ then
            self.potChipData_ = nil
...
function MatchBetChipView:setPotChipData()
...
-- 移动至奖池
function MatchBetChipView:moveToPot()
        ...
        for i, chipData in ipairs(self.potChipData_) do
            ...
            if i > 1 then
                transition.execute(
                    sp, 
                    cc.MoveTo:create(
                        MatchBetChipView.MOVE_TO_POT_DURATION, 
                        cc.p(PP[1].x - 38, PP[1].y + (i - 1) * GAP_WITH_CHIPS)
                    ), 
...
function MatchBetChipView:moveToPotComplete_()
    if self.potChipData_ == self.betChipData_ then
...
function MatchBetChipView:reset()
    -- 回收筹码数据
    if self.lastBetChipData_ then
...
-- 清理
function MatchBetChipView:dispose()
    self:reset()
end

return MatchBetChipView