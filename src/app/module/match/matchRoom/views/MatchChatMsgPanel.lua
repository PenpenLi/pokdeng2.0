...
--
local ChatMsgShortcutListItem = import(".MatchChatMsgShortcutListItem")
local ChatMsgHistoryListItem = import(".MatchChatMsgHistoryListItem")
local ChatTabPanel = import(".MatchChatTabPanel")
local MatchChatMsgPanel = class("MatchChatMsgPanel", ChatTabPanel)
...
function MatchChatMsgPanel:ctor()
    MatchChatMsgPanel.super.ctor(self, bm.LangUtil.getText("ROOM", "CHAT_TAB_SHORTCUT"), bm.LangUtil.getText("ROOM", "CHAT_TAB_HISTORY"))
    ...
    self.editBox_ = cc.ui.UIInput.new({
        ...
        size = cc.size(295, 58),
        x = - MatchChatMsgPanel.PAGE_WIDTH * 0.5 + 212,
    ...
    self.laSelectImage_ = cc.ui.UICheckBoxButton.new({off="#room-chat-pane-laba-background-up.png",on = "#room-chat-pane-laba-background-up.png"})
        ...
        :pos(- MatchChatMsgPanel.PAGE_WIDTH * 0.5 + 32, self.HEIGHT * 0.5 - 8 - 30 - 68)
    ...
    self.laBaNumberLabel_ = display.newTTFLabel({size=14, text="x0", color=cc.c3b(0x27, 0x90, 0xd5)})
        :pos(- MatchChatMsgPanel.PAGE_WIDTH * 0.5 + 55, self.HEIGHT * 0.5 - 8 - 52 - 68)
        :hide()
        :addTo(self)
    ...
    self.sendBtn_ = cc.ui.UIPushButton.new({normal="#common_green_btn_up.png", pressed="#common_green_btn_down.png", disabled="#common_btn_disabled.png"}, {scale9=true})
        ...
        :pos(MatchChatMsgPanel.PAGE_WIDTH * 0.5 - 58, self.editBox_:getPositionY() - 2)
...
function MatchChatMsgPanel:createShortcutPage_()
    local page = display.newNode()

    --快捷消息
    local listW, listH = MatchChatMsgPanel.PAGE_WIDTH, MatchChatMsgPanel.PAGE_HEIGHT - 68
...
function MatchChatMsgPanel:createHistoryPage_()
    local page = display.newNode()
    --快捷消息
    local listW, listH = MatchChatMsgPanel.PAGE_WIDTH, MatchChatMsgPanel.PAGE_HEIGHT - 68
...
function MatchChatMsgPanel:onEnter()
...
function MatchChatMsgPanel:onExit()
...
function MatchChatMsgPanel:historyChanged_(list)
    local mergedList = {}
...
function MatchChatMsgPanel:onEditBoxStateChange_(evt, editbox)
...
function MatchChatMsgPanel:onSendClicked_()
    if nk.userData.silenced == 1 then
        nk.TopTipManager:showTopTip(bm.LangUtil.getText("COMMON", "USER_SILENCED_MSG"))
        return
    end
...
function MatchChatMsgPanel:selectChangeListener(evt)
...
function MatchChatMsgPanel:laSelectImageHandler_()
   if  self.laBaNumber_ and self.laBaNumber_ <= 0 then
            nk.ui.Dialog.new({
            hasCloseButton = true,
