...
--
local HandCard = import(".MatchHandCard")
...
local MatchSeatView = class("MatchSeatView", function() 
    return display.newNode()
end)

MatchSeatView.CLICKED = "MatchSeatView.CLICKED"
MatchSeatView.WIDTH = 108
MatchSeatView.HEIGHT = 164

function MatchSeatView:ctor(ctx, seatId)
    self:retain()
    self.ctx = ctx
    cc(self):addComponent("components.behavior.EventProtocol"):exportMethods()
    self:setNodeEventEnabled(true)
    --2 桌子背景
    --self.background_ = display.newScale9Sprite("#room_seat_bg.png", 0, 0, cc.size(MatchSeatView.WIDTH, MatchSeatView.HEIGHT)):addTo(self, 2)
    self.background_ = display.newSprite("#room_seat_bg.png"):addTo(self, 2)
    --3 赢牌座位金色边框
    self.winBorderBatch_ = display.newBatchNode("room_texture.png", 3):addTo(self, 3, 3):hide()
     self.winBorderBatch_:scale(.82)
    ...
    --坐下图片
    self.sitdown_ = display.newSprite("#room_sitdown_img.png")
        :addTo(self.usrSeatHeadNode_, 1, 1)
    --用户头像容器

    self.image_ = nk.ui.ClipImage.new("#room_girl_icon.png", "#room_black_circle.png")
        :addTo(self.usrSeatHeadNode_, 2, 2)

    --5 头像灰色覆盖
    local usrHeadCover = display.newRect(100, 100, {fill = true, fillColor = cc.c4f(0, 0, 0, 0.6)})
    self.image_:addCoverLayer(usrHeadCover)

    self.nameBg_ = display.newSprite("#room_seat_name_bg.png"):addTo(self, 5, 5)
        :pos(0, -58)


    --6 状态文字
    if appconfig.LANG == "vn" then
        self.state_ = display.newTTFLabel({text = "Seat" .. seatId, size = 19, align = ui.TEXT_ALIGN_CENTER, color=cc.c3b(0xff, 0xff, 0xff) })
    else
        self.state_ = display.newTTFLabel({text = "Seat" .. seatId, size = 19, align = ui.TEXT_ALIGN_CENTER, color=cc.c3b(0xff, 0xff, 0xff) })
    end
    self.state_:pos(0, -47):addTo(self, 6, 6)

    self.chipIcon_ = display.newSprite("#room_pointIcon.png")
        :addTo(self,7)

    local chipIconSize = self.chipIcon_:getContentSize()
    local chipIconW,chipIconH = chipIconSize.width,chipIconSize.height
    local chipIconX,chipIconY = -MatchSeatView.WIDTH*0.5 + chipIconW*0.5+2,-65
    self.chipIcon_:pos(chipIconX,chipIconY)


    --7 座位筹码文字
    self.chips_ = display.newTTFLabel({text = "", size = 19, align = ui.TEXT_ALIGN_LEFT, color=cc.c3b(0xff, 0xff, 0xff)})
        :pos(chipIconX+10, -66)
    ...
    if nk.config.GIFT_SHOP_ENABLED then
           self.giftImage_ = cc.ui.UIPushButton.new({normal = "#gift-icon-up.png",pressed = "#gift-icon-down.png"}):addTo(self, 9, 10):hide()
            :onButtonClicked(buttontHandler(self, self.openGiftPopUpHandler))
    end
    

    --9 winner动画
    self.winnerAnimBatch_ = display.newBatchNode("room_texture.png", 8):addTo(self, 10, 9):hide()
    --winner文字
    ...
    --13 倍数
    self.XIcon_ = display.newNode():pos(100 + 50, -20 + 50):addTo(self, 13):hide()
...
function MatchSeatView:playSitDownAnimation(onCompleteCallback)
...
function MatchSeatView:playStandUpAnimation(onCompleteCallback)
...
function MatchSeatView:fade()
    self.image_:setImgCoverShowState(true)
    transition.execute(self.image_:getImgCover(), cc.FadeIn:create(0.2))
end

function MatchSeatView:unfade()
    self.image_:setImgCoverShowState(false)
end

function MatchSeatView:sitDownState_()
...
function MatchSeatView:standUpState_()
    self.image_:stopAllActions()
    self.sitdown_:stopAllActions()
...
function MatchSeatView:isEmpty()
    return not self.seatData_
end

function MatchSeatView:getPositionId()
    return self.positionId_
end

function MatchSeatView:setPositionId(id)
    self.positionId_ = id
    if nk.config.GIFT_SHOP_ENABLED then
        if id then
            if id == 1 or id == 2 or id == 3 or id == 4 then
                self.giftImage_:pos(52, 0)
            else
                self.giftImage_:pos(- 52, 0)
            end
        end
...
function MatchSeatView:resetToEmpty()
    self.seatData_ = nil
    self:updateState()
end

function MatchSeatView:setSeatData(seatData)
    ...
    if not seatData then
        ...
        if seatData.userInfo.msex == "2" then
            self.image_:setImgToClipStr("#common_female_avatar.png")
        else
            self.image_:setImgToClipStr("#common_male_avatar.png")
        end
...
function MatchSeatView:getSeatData()
    return self.seatData_
end


function MatchSeatView:userGiftImageLoadCallback_(success, sprite)
    if success then
...
function MatchSeatView:updateGiftUrl(gift_Id)
    if nk.config.GIFT_SHOP_ENABLED then
        if self.giftUrlReqId_ then
            LoadGiftControl:getInstance():cancel(self.giftUrlReqId_)
        end
...
function MatchSeatView:updateHeadImage(imgurl)
...
function MatchSeatView:updateState()
    -- dump(self.seatData_, "SeatView:updateState111")
    -- dump(self.ctx.model:isSelfInSeat(), "SeatView:updateState222")
    -- if self.forceHide_ then
    --     self:hide()
    --     return
    -- end
    ...
    if self.seatData_ == nil then
        ...
        if (st ~= SeatStateMachine.STATE_BETTING and st ~= SeatStateMachine.STATE_GETTING) then
...
function MatchSeatView:setHandCardValue(cards)
    if cards then
        printf("MatchSeatView[%s]setHandCardValue [%x %x %x][%s %s %s][%s %s %s]", self.seatId_ or -1, cards[1] or 0, cards[2] or 0, cards[3] or 0, cards[1] or 0, cards[2] or 0, cards[3] or 0, nk.getCardDesc(cards[1]), nk.getCardDesc(cards[2]), nk.getCardDesc(cards[3]))
    end
    self.handCards_:setCards(cards)
end

function MatchSeatView:setHandCardNum(num)
    self.handCards_:setCardNum(num)
end

function MatchSeatView:showHandCards()
    self.handCards_:show()
end

function MatchSeatView:hideHandCards()
    self.handCards_:hide()
end

function MatchSeatView:showHandCardBackAll()
    self.handCards_:showBackAll()
end

function MatchSeatView:showHandCardFrontAll()
    self.handCards_:showFrontAll()
end

function MatchSeatView:flipAllHandCards()
    self.handCards_:flipAll()
end

function MatchSeatView:hideAllHandCardsElement()
    self.handCards_:hideAllCards()
end

function MatchSeatView:showAllHandCardsElement()
    self.handCards_:showAllCards()
end

function MatchSeatView:showHandCardsElement(idx)
    self.handCards_:showWithIndex(idx)
end

function MatchSeatView:flipHandCardsElement(idx)
    self.handCards_:flipWithIndex(idx)
end

function MatchSeatView:shakeAllHandCards()
    self.handCards_:shakeWithNum(3)
end

function MatchSeatView:setForceHide(forceHide)
    self.forceHide_ = forceHide
end

function MatchSeatView:shadowAllHandCards()
    -- body
    self.handCards_:shadowAllCards()
end

-- 三公结束亮出自己手牌在博定中无用
function MatchSeatView:showHandCardsAnimation()
...
function MatchSeatView:showCardTypeIf()
    ...
    if self.seatData_ and self.seatData_.handPoker and self.seatData_.handPoker:getTypeLabel() then
        ...
        self.cardType_:show()
        if self.seatData_.handPoker:isPokdeng() and (self.seatData_.isOutCard == 1) then
            self.pokdengIcon_.showIcon(self.seatData_.handPoker:getPoint())
        end

        if self.seatData_.isOutCard == 1 then
...
function MatchSeatView:playExpChangeAnimation(expChange)
    if expChange > 0 then
...
function MatchSeatView:playAutoBuyinAnimation(buyinChips)
    local buyInBg = display.newSprite("#buyin-action-yellowbackground.png")
        :addTo(self, 6)
    local buyInBgSize = buyInBg:getContentSize()
    buyInBg:pos(0, -MatchSeatView.HEIGHT/2 + buyInBgSize.height/2)
    ...
    local buyInLabel = display.newTTFLabel({
            ...
            size = 32, 
            align = ui.TEXT_ALIGN_CENTER
        }):addTo(self, 6):pos(0, -MatchSeatView.HEIGHT/2 + buyInBgSize.height/2 + buyInLabelPaddding)
    ...
    local buyInLabelSequence = transition.sequence({
            spawn({
                ...
                cc.MoveTo:create(1, cc.p(0, MatchSeatView.HEIGHT/2 - buyInBgSize.height/2 - buyInLabelPaddding)),
            }),
            cc.CallFunc:create(function()
...
function MatchSeatView:playWinAnimation()
...
function MatchSeatView:stopWinAnimation_()
    self.winBorderBatch_:hide()
    self.winnerAnimBatch_:hide()
    self.winner_:stopAllActions()
    self.star1_:stopAllActions()
...
function MatchSeatView:onTouch_(target, evt)
    if evt == bm.TouchHelper.CLICK then
        nk.SoundManager:playSound(nk.SoundManager.CLICK_BUTTON)
        self:dispatchEvent({name=MatchSeatView.CLICKED, seatId=self.seatId_, target=self})
    end
end

function MatchSeatView:reset()
    self.handCards_:showAllCards()
    self.handCards_:showBackAll()
...
function MatchSeatView:openGiftPopUpHandler()
    local roomUid = ""
    ...
    if self.ctx.model.playerList[self.seatId_] then
        if self.ctx.model.playerList[self.seatId_].uid == nk.userData.uid then
            ...
            UserInfoPopup.new(3):show(true, tableInfo)
        else
            local inRoomInfo = {}
...
function MatchSeatView:onCleanup()
...
function MatchSeatView:dispose()
