...
local MatchUserInfoOtherDialog = class("MatchUserInfoOtherDialog", function() return nk.ui.Panel.new({WIDTH, HEIGHT}) end)

function MatchUserInfoOtherDialog:ctor(ctx)
...
function MatchUserInfoOtherDialog:getFriendDataByUid(friendUid)
    -- body
...
function MatchUserInfoOtherDialog:loadFriendUidList()
...
function MatchUserInfoOtherDialog:show(data)
    self:setData(data)
    self:showPanel_(true, true, true, true)
end

function MatchUserInfoOtherDialog:hide()
...
function MatchUserInfoOtherDialog:setAddFriendStatus()
...
function MatchUserInfoOtherDialog:setData(data)
    self.data_ = data
    if data then
        -- dump(data, "MatchUserInfoOtherDialog:setData.data :====================")
...
function MatchUserInfoOtherDialog:setImage_(sprite)
...
function MatchUserInfoOtherDialog:imageLoadCallback_(success, sprite)
    if success then
...
function MatchUserInfoOtherDialog:onFriendClicked_(evt)
...
function MatchUserInfoOtherDialog:onAddFriendClicked_(evt)
...
function MatchUserInfoOtherDialog:onDelFriendClicked_(evt)
...
function MatchUserInfoOtherDialog:sendChipClicked_(chips)
    if self.ctx.model:isSelfInSeat() then
        local roomData = nk.getRoomDataByLevel(self.ctx.model.roomInfo.roomType)
        ...
        if (nk.userData["aUser.money"] -  chips ) > roomData.minBuyIn then
               nk.http.sendCoinForRoom(nk.userData.uid,self.data_.uid, chips,
                   function (data)
...
function MatchUserInfoOtherDialog:sendHddjClicked_(hddjId)
...
function MatchUserInfoOtherDialog:doSendHddj()
...
function MatchUserInfoOtherDialog:sendHddjAndHide_()
...
function MatchUserInfoOtherDialog:showHddjNotEnoughDialog_()
    nk.ui.Dialog.new({
...
function MatchUserInfoOtherDialog:onCleanup()
    if self.addFriendRequistId_ then
...
function MatchUserInfoOtherDialog:onExit()
    nk.ImageLoader:cancelJobByLoaderId(self.headImageLoaderId_)
...
return MatchUserInfoOtherDialog