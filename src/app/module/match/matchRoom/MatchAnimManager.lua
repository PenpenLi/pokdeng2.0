...
--
local MatchAnimManager = class("MatchAnimManager")
local scheduler = require(cc.PACKAGE_NAME .. ".scheduler")
local HddjController = import(".MatchHddjController")
local YouWinAnim = import(".views.MatchYouWinAnim")
local RoomViewPosition = import(".views.MatchRoomViewPosition")
local SendChipView = import(".views.MatchSendChipView")
local RoomChatBubble = import(".views.MatchRoomChatBubble")
local RoomSignalIndicator = import(".views.MatchRoomSignalIndicator")
-- local ExpressionConfig = import(".views.MatchExpressionConfig").new()
...
local WinCardAnim = import(".views.MatchWinCardAnim")
...
local SpeakerPosition = RoomViewPosition.SpeakerPosition

function MatchAnimManager:ctor()
    
end

function MatchAnimManager:createNodes()
    self.tableDealerPositionId_ = 10
    self.tableDealer_ = display.newSprite("#room_table_dealer_2.png")
    ...
    self.dealerAnimNode_ = display.newNode():addTo(self.ctx.scene.nodes.dealerNode) 
    ...
    self.tableSpeakerPositionId_ = 10
     self.tableSpeaker_ = display.newSprite("#room_table_speaker_2.png")
        :pos(SpeakerPosition[self.tableSpeakerPositionId_].x, SpeakerPosition[self.tableSpeakerPositionId_].y)
        :addTo(self.ctx.scene.nodes.dealerNode)
        :hide()
    self.speakerAnimNode_ = display.newNode():addTo(self.ctx.scene.nodes.dealerNode)
    :hide() 

    -- self.pokerBatch_ = display.newBatchNode("room_texture.png"):addTo(self.ctx.scene.nodes.dealerNode)
    -- self.pokerBatch_:pos(
    --     DealCardStartPosition[self.tableSpeakerPositionId_].x, 
    --     DealCardStartPosition[self.tableSpeakerPositionId_].y)

    self.signal_ = RoomSignalIndicator.new()
        :addTo(self.ctx.scene.nodes.dealerNode)
        :pos(display.cx - 86 + 10, display.top - 32)

    self.clock_ = display.newTTFLabel({size=20, color=cc.c3b(0x2B, 0x56, 0x86), text=os.date("%H:%M", os.time()), align=ui.TEXT_ALIGN_CENTER})
        :addTo(self.ctx.scene.nodes.dealerNode)
...
-- 发光
function MatchAnimManager:lightDealer_()    
    -- self.dealerAnimNode_:performWithDelay(function ()
    --      self.tableDealer_:setSpriteFrame(display.newSpriteFrame("room_table_dealer_1.png"))
    -- end, 1)
    -- self.dealerAnimNode_:performWithDelay(function ()
    --      self.tableDealer_:setSpriteFrame(display.newSpriteFrame("room_table_dealer_2.png"))
    -- end, 2)
    -- self.dealerAnimNode_:performWithDelay(function ()
    --      self:lightDealer_()
    -- end, 3)
...
function MatchAnimManager:onSignalStrengthChanged_(strength)

    if self.signal_ and self.signal_.setSignalStrength then
        --todo
...
function MatchAnimManager:moveDealerTo(positionId, animation)   
...
function MatchAnimManager:rotateDealer(step)
...
-- positionId = 10 移动到荷官位置
function MatchAnimManager:moveSpeakerTo(positionId, animation)   
    if positionId == nil then
        positionId = 10
    end
    if positionId == 10 then        
        self.tableSpeaker_:setSpriteFrame(display.newSpriteFrame("room_table_speaker_2.png"))
    else
        self.tableSpeaker_:setSpriteFrame(display.newSpriteFrame("room_table_speaker_1.png"))
    end

    local p = SpeakerPosition[positionId]
    local p2 = SpeakerPosition[positionId]
    if not p then
        p = SpeakerPosition[10]
        p2 = DealCardStartPosition[10]
        self.tableSpeakerPositionId_ = 1
    end
    self.tableSpeaker_:stopAllActions()
    self.pokerBatch_:stopAllActions()
    if animation then
        self.tableSpeaker_:moveTo(0.5, p.x, p.y)
        self.pokerBatch_:moveTo(0.5, p2.x, p2.y)
    else
        self.tableSpeaker_:setPosition(p)
        self.pokerBatch_:setPosition(p2)
    end    
    self.tableSpeakerPositionId_ = positionId
end

function MatchAnimManager:rotateSpeaker(step)
    if self.tableSpeakerPositionId_ == 10 then
        return
    end
    local newPositionId = self.tableSpeakerPositionId_ - step
    if newPositionId > 9 then
        newPositionId = newPositionId - 9
    elseif newPositionId < 1 then
        newPositionId = newPositionId + 9
    end
    self:moveSpeakerTo(newPositionId, true)
end



function MatchAnimManager:playYouWinAnim()
...
function MatchAnimManager:playWinCardAnim()
    do return end
    if not self.winCardAnim_ then
        self.winCardAnim_ = WinCardAnim.new():pos(display.cx, SeatPosition[1].y - 174)
        self.winCardAnim_:retain()
    end
    if self.winCardAnim_:getParent() then
        if self.winCardScheduleHandle_ then
            scheduler.unscheduleGlobal(self.winCardScheduleHandle_)
        end
    else
        self.winCardAnim_:show()
        self.winCardAnim_:addTo(self.scene.nodes.animNode)
    end
    self.winCardScheduleHandle_ = scheduler.performWithDelayGlobal(handler(self, function (obj)
        obj.winCardAnim_:removeFromParent()
    end), 10)
end

function MatchAnimManager:playAddFriendAnimation(fromSeatId, toSeatId)
...
function MatchAnimManager:playSendChipAnimation(fromSeatId, toSeatId, chips)
...
function MatchAnimManager:playSendGiftAnimation(giftId, fromUid, toUidArr)
...
function MatchAnimManager:checkUidInSeat(uidArr)
...
function MatchAnimManager:playHddjAnimation(fromSeatId, toSeatId, hddjId)
...
function MatchAnimManager:playExpression(seatId, expressionId)
...
function MatchAnimManager:playExpressionAnim_(seatId, expressionId, anim)
    if self.model.playerList[seatId] then
        local config = ExpressionConfig:getConfig(expressionId)
        ...
        local animTime = config.frameNum / 3 > 3 and config.frameNum / 3 or 3
        local playTime = animTime + 0.5  -- 延迟0.5s释放动画
...
--座位扣钱动画
function MatchAnimManager:playChipsChangeAnimation(seatId, chipsChange)
    ...
    if chipsChange > 0 then
        local lb = display.newTTFLabel({size=24, text=string.format("+$%s", math.abs(chipsChange)), color=cc.c3b(0xEC, 0xCE, 0x0B)})
...
--显示聊天消息
function MatchAnimManager:showChatMsg(seatId, message)
...
function MatchAnimManager:removeAllAnimation()
...
function MatchAnimManager:dispose()
    if self.youWinScheduleHandle_ then
        scheduler.unscheduleGlobal(self.youWinScheduleHandle_)
    end
...
function MatchAnimManager:bindDataObservers_()
