...
-- Date: 2016-05-30 14:0:39
-- Copyright: Copyright (c) 2015, BOYAA INTERACTIVE CO., LTD All rights reserved.
-- 比赛结果(定时赛)

local MatchTimeGameResultPopup = class("MatchTimeGameResultPopup", nk.ui.Panel)
local WIDTH = 558
local HEIGHT = 328
local BTN_WIDTH = 143
local BTN_HEIGHT = 53
function MatchTimeGameResultPopup:ctor(rank,matchType,resultData,callback)
...
function MatchTimeGameResultPopup:createNode_()
	-- body
    ...
    if self.resultData_.msg == nil or self.resultData_.msg == "" then
    ...
    if self.resultData_.msg then
        self.awardtxt_:setString(self.resultData_.msg)
    end
...
function MatchTimeGameResultPopup:show()
	self:showPanel_()
end

function MatchTimeGameResultPopup:onCleanup()
	self.callback_ = nil
    display.removeSpriteFramesWithFile("match.plist", "match.png")
end
function MatchTimeGameResultPopup:shareBtnClk()
...
function MatchTimeGameResultPopup:applyBtnClk()
   if self.callback_ then
        self.callback_(3)
    end
    self:onClose()
end
return MatchTimeGameResultPopup