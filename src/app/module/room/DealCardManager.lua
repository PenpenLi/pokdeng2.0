...
function DealCardManager:dealCard_(positionId)
    ...
    if dealingcard:getNumberOfRunningActions() == 0 then
        dump(self.ctx.model.gameInfo.dealerSeatId,"self.ctx.model.gameInfo.dealerSeatId")
...
function DealCardManager:showDealedCard(player, cardNum)    
    ...
    for i = 1, cardNum do
        ...
        if i <= cardNum then
            deadCard:addTo(self.cardBatchNode_):pos(P[positionId].x + i * 8 - 16, P[positionId].y):rotation(i * 12 - 24 + rotatAddAngle)
        end
    end
end
