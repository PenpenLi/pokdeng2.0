...
local SeatProgressTimer = import(".views.SeatProgressTimerEx")
...
function SeatManager:onSeatClicked_(evt)
    ...
    --[[
    do
        self.hddjId_ = self.hddjId_ and (self.hddjId_ + 1) or 1
        if self.hddjId_ == 11 then
            self.hddjId_ = 1
        end
        self.animManager:playHddjAnimation(1, evt.seatId, self.hddjId_)
        return
    end
    ]]
    ...
    if seat:isEmpty() then
        ...
        if nk.userData["aUser.money"] < self.model.roomInfo.minBuyIn then
            ...
            roomDataFilter.roomType = roomData and roomData.roomGroup or nil
