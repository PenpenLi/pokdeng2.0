...
function NormalRoomBaseView:ctor(ctx)
    ...
    if nk.config.CHRISTMAS_THEME_ENABLED then
        ...
        for i = 1, 4 do
            local snowWindwEffect = display.newSprite("#christ_roomSnowCorner_right.png")
                :rotation(90 * i)
    ...
    if not isOpen then
        ...
        if self._firstChargeEntryBtn then
            ...
            if nk.OnOff:check("rechargeFavGray") and isShowPay and isPay and rechargeFavAccess then
                ...
                self._almRechargeEntryBtn = cc.ui.UIPushButton.new({normal = reChargeIconResKey[1], pressed = reChargeIconResKey[2], disabled = reChargeIconResKey[3]},
                    {scale9 = false})
                    :onButtonClicked(buttontHandler(self, self.onAlmRechargeFavCallBack_))
                    :pos(shopPosX, shopPosY)
                    :addTo(self.topNode_)
...
function NormalRoomBaseView:refreshAlmRechargeFavEntryUi(isOpen)
    -- body
    if not isOpen then
        --todo
