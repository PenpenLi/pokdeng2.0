...
function RaiseSlider:ctor()
    ...
    for i = 1, 412 / 4 do
        local orgX, orgY = - 4, i * 4 - 209
        self.trackHLineBatch_:drawRect({x = orgX, y = orgY}, {x = orgX + 7, y = orgY + 2}, {fillColor = cc.c3b(0x32, 0x46, 0x39)})
    end
    ...
    self.label_ = display.newTTFLabel({text="$0", size=24, color=cc.c3b(0xc4, 0xca, 0xd4)})
    ...
    self.btnAllin_ = nk.ui.SimpleButton.new({
        ...
        disabled="down"
    }, 5):onClicked(handler(self, self.onButtonClicked_)):pos(self.labelBackground_:getPositionX(), self.labelBackground_:getPositionY()):enabled(true):addTo(self)
...
function RaiseSlider:onSliderPercentValueChanged_(newVal, forceUpdate, needSound)
    if self.percentValue_ ~= newVal or forceUpdate then
        ...
        if self.curValue_ > 9999 then
            self.curValue_ = bm.formatBigNumber(self.curValue_)
        end
