...
function SideBar:ctor(callback)
    ...
    --触摸区域
    local touchModleSizeAdjust = {
        width = 10,
        height = 20
    }
    
    local touchWidth = sideBtnWidth * 2 + touchModleSizeAdjust.width
    local touchHeight = sideBtnHeight + 40 + touchModleSizeAdjust.height
    self.touchNode_ = display.newScale9Sprite("#common_modTransparent.png"):size(touchWidth, touchHeight):addTo(self)
