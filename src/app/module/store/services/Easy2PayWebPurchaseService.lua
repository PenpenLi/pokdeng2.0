...
local Easy2PayWebPurchaseService = class("Easy2PayWebPurchaseService", import("app.module.store.PurchaseServiceBase"))
...
function Easy2PayWebPurchaseService:ctor()
    self.payStatistics_ = {}
    Easy2PayWebPurchaseService.super.ctor(self, "Easy2PayWebPurchaseService")
    self.helper_ = PurchaseHelper.new("Easy2PayWebPurchaseService")
...
function Easy2PayWebPurchaseService:init(config)
    self.config_ = config
    self.active_ = true
...
function Easy2PayWebPurchaseService:autoDispose()
    self.active_ = false
...
function Easy2PayWebPurchaseService:loadChipProductList(callback)
...
--callback(payType, isComplete, data)
function Easy2PayWebPurchaseService:loadPropProductList(callback)
    self.loadPropCallback_ = callback
    self.loadPropRequested_ = true
    self:loadProcess_()
end

function Easy2PayWebPurchaseService:loadTicketProductList(callback)
    self.loadTicketCallback_ = callback
    self.loadTicketRequested_ = true
    self:loadProcess_()
end

function Easy2PayWebPurchaseService:loadCashProductList(callback)
    -- body
...
function Easy2PayWebPurchaseService:loadProcess_()
...
function Easy2PayWebPurchaseService:invokeCallback_(flag, isComplete, data)
...
function Easy2PayWebPurchaseService:configLoadHandler_(succ, content)
    if succ then
...
function Easy2PayWebPurchaseService:makePurchase(pid, callback,goodData)
    ...
    params.channel = "psms"
    params.uid = tostring(nk.userData.uid)
...
function Easy2PayWebPurchaseService:hidePayWebView()
...
function Easy2PayWebPurchaseService:onWebViewBgTouch()
    self:hidePayWebView();
end


function Easy2PayWebPurchaseService:showPayWebView(webUrl)
  
    if not self.webView_ then
        self.webView_ = ccexp.WebView:create()
        self.webView_:size(860, 614)
        self.webView_:addTo(nk.runningScene, 1001)
        self.webView_:setScalesPageToFit(true)
        self.webView_:pos(display.cx, display.cy)

        local webViewBg = display.newColorLayer(ccc4(0, 0, 0, 128)):addTo(self.webView_)
        webViewBg:setTouchEnabled(true)
        webViewBg:setTouchSwallowEnabled(true)
        webViewBg:addNodeEventListener(cc.NODE_TOUCH_EVENT, handler(self, self.onWebViewBgTouch))
...
function Easy2PayWebPurchaseService:onPayResult_(jsonString)
    self.isPurchasing_ = false
...
function Easy2PayWebPurchaseService:toptip(msg)
    nk.TopTipManager:showTopTip(msg)
end

return Easy2PayWebPurchaseService