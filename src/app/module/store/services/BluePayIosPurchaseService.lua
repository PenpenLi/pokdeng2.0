...
local PURCHASE_CATEGORY = import("..managers.PURCHASE_CATEGORY")
local PurchaseHelper = import("app.module.store.managers.PurchaseHelper")
local BluePayIosPurchaseService = class("BluePayIosPurchaseService", import("app.module.store.PurchaseServiceBase"))

function BluePayIosPurchaseService:ctor()
    BluePayIosPurchaseService.super.ctor(self, "BluePayIosPurchaseService")

    self.helper_ = PurchaseHelper.new("BluePayIosPurchaseService")

    if device.platform == "ios" then
        self.invokeOcMethod_ = self:createOcMethodInvoker("BluePayBridge")
        self.invokeOcMethod_("setSetupCompleteCallback", {listener = handler(self, self.onSetupComplete_)})
        self.invokeOcMethod_("setPurchaseCompleteCallback", {listener = handler(self, self.onPurchaseComplete_)})
    else
        self.invokeOcMethod_ = function(method, param, sig)
...
function BluePayIosPurchaseService:init(config)
    dump(config,"BluePayIosPurchaseService:init")
    self.config_ = config
    self.active_ = true
    local success, ret = self.invokeOcMethod_("isSetupComplete",nil)
    if success then
        self.isSetupComplete_ = ret
    end
    success, ret = self.invokeOcMethod_("isSupported", nil)
    if success then
    ...
    if not self.isSetupComplete_ then
        ...
        self.invokeOcMethod_("setup", nil)
    end
...
function BluePayIosPurchaseService:autoDispose()
    self.active_ = false
...
function BluePayIosPurchaseService:loadChipProductList(callback)
    self.loadChipCallback_ = callback
    self.loadChipRequested_ = true
    self:loadProcess_()
end

function BluePayIosPurchaseService:loadTicketProductList(callback)
    self.loadTicketCallback_ = callback
    self.loadTicketRequested_ = true
    self:loadProcess_()
end

function BluePayIosPurchaseService:loadCashProductList(callback)
    -- body
...
--callback(payType, isComplete, data)
function BluePayIosPurchaseService:loadPropProductList(callback)
    self.loadPropCallback_ = callback
    self.loadPropRequested_ = true
    self:loadProcess_()
end

function BluePayIosPurchaseService:makePurchase(pid, callback,goodData)
...
function BluePayIosPurchaseService:payBySMS(uid,pid,transactionId,currency,price,smsId,propsName,isShowDialog)
    self.invokeOcMethod_("payBySMS", {uid = uid,pid = pid,transactionId = transactionId,currency = currency,price = price,smsId = smsId,propsName = propsName,isShowDialog = isShowDialog})
end



function BluePayIosPurchaseService:payByCashcard(uid,pid,transactionId,propsName,publicer,cardNo,serialNo)
    self.invokeOcMethod_("payByCashcard", {uid,pid,transactionId,propsName,publicer,cardNo,serialNo})
end

function BluePayIosPurchaseService:payBySteps(uid,pid,transactionId,currency,price,smsId,propsName,mode,isShowDialog)
    self.invokeOcMethod_("payBySteps", {uid,pid,transactionId,currency,price,smsId,propsName,mode,isShowDialog})
end

function BluePayIosPurchaseService:payByBank(uid,pid,transactionId,currency,price,propsName,isShowDialog)
    self.invokeOcMethod_("payByBank", {uid,pid,transactionId,currency,price,propsName,isShowDialog})
end


--加载商品信息流程
function BluePayIosPurchaseService:loadProcess_()
    if not self.products_ then
        self.logger:debug("remote config is loading..")
        self.helper_:cacheConfig(self.config_.configURL, handler(self, self.configLoadHandler_))
...
--Java call lua
function BluePayIosPurchaseService:onSetupComplete_(isSupported)
...
--Java call lua
function BluePayIosPurchaseService:onLoadProductsComplete_(jsonString)
...
--Java call lua
function BluePayIosPurchaseService:onPurchaseComplete_(jsonString)
...
function BluePayIosPurchaseService:invokeCallback_(flag, isComplete, data)
...
function BluePayIosPurchaseService:filterProducts(datas)
...
function BluePayIosPurchaseService:configLoadHandler_(succ, content)
    if succ then
...
function BluePayIosPurchaseService:reportBluePayInfo(jsonData)
...
return BluePayIosPurchaseService
