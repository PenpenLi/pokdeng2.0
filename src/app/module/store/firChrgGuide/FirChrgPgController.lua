...
--
local QuickPurchaseServiceManager = import("..managers.QuickPurchaseServiceManager")
local PURCHASE_TYPE = import("..managers.PURCHASE_TYPE")
...
function FirChrgPgController:onConfigLoaded(ret, paytypeConfig)
	...
	if ret ~= 0 then
		...
		if self.view_ and self.view_.setLoadingBarEnabled then
			--todo
			self.view_:setLoadingBarEnabled(false)
		end
	end
end
...
function FirChrgPgController:onPurchaseResultCallBack(isSucc, result)
	...
	if isSucc then
		--todo
		nk.userData.best.paylog = 1  -- Charge Record
		nk.userData.best.ispay = 1  -- Chip Charge Record
