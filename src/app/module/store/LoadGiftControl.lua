...
-- Description: LoadGiftControl.lua Reconstruced By Tsing.
-- 
    ...
    if not self.isConfigLoaded_ and not self.isConfigLoading_ then
        ...
        bm.cacheFile(self.url_ or nk.userData.GIFT_JSON, function(result, content)
            ...
            if result == "success" then
                ...
                -- dump(self.giftData_, "self.giftData_ :==============")
...
function LoadGiftControl:getGiftNameById(giftId, callback)
    -- body
    if self.isConfigLoaded_ then
        if self.giftData_ then
            for i=1,#self.giftData_ do
                if tonumber(self.giftData_[i].pnid) == tonumber(giftId) then
                    ...
                    -- dump(self.giftData_[i], "self.giftData_[i]:================")
                    if callback then
                        callback(self.giftData_[i].name)
                        return nil
                    end
                end
            end
