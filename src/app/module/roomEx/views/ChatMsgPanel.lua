...
function ChatMsgPanel:ctor()
    ...
    self.inputBg_ = display.newSprite("#room_chat_input_bg.png")
    :addTo(self)
    :pos(0,-self.HEIGHT * 0.5 + 33)

   

    self.suonaBg_ = display.newSprite("#room_chat_souna_bg.png")
    :addTo(self)
    :pos(-self.WIDTH/2 + 64/2+5,-self.HEIGHT * 0.5 + 32)

    -- self.inputLine_ = display.newSprite("#room_chat_input_line.png")
    -- :addTo(self)
    -- :pos(-self.WIDTH/2 + 226/2+64+4,-self.HEIGHT * 0.5 + 32)

    self.sendBtnBg_ = display.newSprite("#room_chat_send_btn.png")
    :addTo(self)
    :pos(self.WIDTH/2 - 96/2-7,-self.HEIGHT * 0.5 + 32)

    
    self.suonaBtn_ = cc.ui.UIPushButton.new("#room_chat_suona_icon.png",{scale9 = false})
    :addTo(self)
    :pos(-self.WIDTH/2 + 64/2+2,-self.HEIGHT * 0.5 + 32)
    :onButtonClicked(function()
            SuonaUsePopup.new():show()
        end)

    --发送按钮
    self.sendBtn_ = cc.ui.UIPushButton.new({normal = "#common_transparent_skin.png", pressed = "#rounded_rect_10.png"}, {scale9=true})
    ...
    self.editBox_ = ui.newEditBox({
        image = "#room_chat_input_line.png",
        imagePressed = "#room_chat_input_line.png",
        size = CCSize(226, 57),
        x = -self.WIDTH/2 + 226/2+64+4,
        y = -self.HEIGHT * 0.5 + 32,
    ...
    self.editBox_:setFontColor(cc.c3b(0x0f, 0x1d, 0x35))
    self.editBox_:setPlaceholderFontName(ui.DEFAULT_TTF_FONT)
    self.editBox_:setPlaceholderFontSize(25)
    self.editBox_:setPlaceholderFontColor(cc.c3b(0x0f, 0x1d, 0x35))
    ...
    self.laBaNumberLabel_ = ui.newTTFLabel({size=14, text="x0", color=cc.c3b(0x27, 0x90, 0xd5)})
...
function ChatMsgPanel:createShortcutPage_()
    ...
    --快捷消息
    local listW, listH = ChatMsgPanel.PAGE_WIDTH, ChatMsgPanel.PAGE_HEIGHT - 78
...
function ChatMsgPanel:selectChangeListener(evt)
    ...
    if evt.target:isButtonSelected()  then
       self.MessageType = 2
       self.laBaImage_:setDisplayFrame(getFrame("common-laba-icon-up.png"))
       self.laBaNumberLabel_:show()
    else
        self.MessageType = 1
        self.laBaImage_:setDisplayFrame(getFrame("common-laba-icon-down.png"))
