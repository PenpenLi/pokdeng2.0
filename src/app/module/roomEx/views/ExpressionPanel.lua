...
ExpressionPanel.WIDTH = 400
ExpressionPanel.HEIGHT  = 637
...
function ExpressionPanel:ctor(ctx, defaultPageIdx)
    ...
    self.background_ = display.newScale9Sprite("#room_card_review_bg.png", 0, 0, CCSize(ExpressionPanel.WIDTH, ExpressionPanel.HEIGHT))
   
    self.background_:addTo(self)
    self.background_:setTouchEnabled(true)
    self.background_:setTouchSwallowEnabled(true)
    self:pos(-ExpressionPanel.WIDTH * 0.5, ExpressionPanel.HEIGHT * 0.5 )
    self:createUI_()
end
...
function ExpressionPanel:createUI_()
    ...
    self.tipsBottom_ = ui.newTTFLabel({text = "labe String", size = tipsBottomLabelParam.frontSize,
    ...
    self.normalPos_ = cc.p(-ExpressionPanel.WIDTH/2+BTN_WIDTH/2,ExpressionPanel.HEIGHT/2-BTN_HEIGHT/2)
    self.normalBtn_ = cc.ui.UIPushButton.new({normal = "#room_exp_tab_up.png", pressed = "#room_exp_tab_up.png"})
        :setButtonLabel("normal", ui.newTTFLabel({text = bm.LangUtil.getText("ROOM", "EXPRE_TABTEXT")[1], color = styles.FONT_COLOR.LIGHT_TEXT, size = 20, align = ui.TEXT_ALIGN_CENTER}))
        :addTo(self)
        :pos(-ExpressionPanel.WIDTH/2+BTN_WIDTH/2+5,ExpressionPanel.HEIGHT/2-BTN_HEIGHT/2)
        :onButtonClicked(handler(self, self.onNorClick_))

    self.huLiPos_ = cc.p(0,ExpressionPanel.HEIGHT/2-BTN_HEIGHT/2)
    self.huLiBtn_ = cc.ui.UIPushButton.new({normal = "#room_exp_tab_up.png", pressed = "#room_exp_tab_up.png"})
        :addTo(self)
        :pos(0,ExpressionPanel.HEIGHT/2-BTN_HEIGHT/2)
        :setButtonLabel("normal", ui.newTTFLabel({text = bm.LangUtil.getText("ROOM", "EXPRE_TABTEXT")[2], color = styles.FONT_COLOR.LIGHT_TEXT, size = 20, align = ui.TEXT_ALIGN_CENTER}))
        :onButtonClicked(handler(self, self.onHuLiClick_))

    self.tuZiPos_ = cc.p(ExpressionPanel.WIDTH/2-BTN_WIDTH/2,ExpressionPanel.HEIGHT/2-BTN_HEIGHT/2)
     self.tuZiBtn_ = cc.ui.UIPushButton.new({normal = "#room_exp_tab_up.png", pressed = "#room_exp_tab_up.png"})
        :addTo(self)
        :pos(ExpressionPanel.WIDTH/2-BTN_WIDTH/2-5,ExpressionPanel.HEIGHT/2-BTN_HEIGHT/2)
        :setButtonLabel("normal", ui.newTTFLabel({text = bm.LangUtil.getText("ROOM", "EXPRE_TABTEXT")[3], color = styles.FONT_COLOR.LIGHT_TEXT, size = 20, align = ui.TEXT_ALIGN_CENTER}))
        :onButtonClicked(handler(self, self.onTuZiClick_))

    self.row_ = display.newSprite("#room_exp_tab_down.png"):addTo(self)
    self.row_:pos(-ExpressionPanel.WIDTH/2+BTN_WIDTH/2+3,ExpressionPanel.HEIGHT/2-BTN_HEIGHT/2 - 3)
    self.row_text_ = ui.newTTFLabel({text = bm.LangUtil.getText("ROOM", "EXPRE_TABTEXT")[1], color = styles.FONT_COLOR.LIGHT_TEXT, size = 21, align = ui.TEXT_ALIGN_CENTER})
    :addTo(self.row_)
    :pos(136/2,66/2)

    self:onExprTabChanged(1)
end
function ExpressionPanel:onNorClick_()
     transition.moveTo(self.row_, {time=0.2, x=self.normalPos_.x+5, y=self.normalPos_.y-3,
         onComplete = function()
            self:onExprTabChanged(1)
            self.row_text_:setString(bm.LangUtil.getText("ROOM", "EXPRE_TABTEXT")[1])
        end,
     easing="OUT"})
end
function ExpressionPanel:onHuLiClick_()
    transition.moveTo(self.row_, {time=0.2, x=self.huLiPos_.x, y=self.huLiPos_.y-3,
         onComplete = function()
            self:onExprTabChanged(2)
            self.row_text_:setString(bm.LangUtil.getText("ROOM", "EXPRE_TABTEXT")[2])
        end,
     easing="OUT"})
end
function ExpressionPanel:onTuZiClick_()
    transition.moveTo(self.row_, {time=0.2, x=self.tuZiPos_.x-5, y=self.tuZiPos_.y-3,
         onComplete = function()
            self:onExprTabChanged(3)
            self.row_text_:setString(bm.LangUtil.getText("ROOM", "EXPRE_TABTEXT")[3])
        end,
     easing="OUT"})
end
function ExpressionPanel:drawExprViewByIdx(idx)
    ...
    local renderViewByIndex = {
        [1] = function()
            ...
            local expNum = 27
            local contentWidth = 385
            local contentHeight = ((expNum / 5 - (expNum / 5) % 1) + (expNum % 5 == 0 and 0 or 1)) * 100
            local contentLeft =  -0.5 * contentWidth - 15
            local contentTop = 0.5 * contentHeight+ 40
            for i = 1, expNum do
                local id = i
        ...
        [2] = function()
            ...
            -- local label = ui.newTTFLabel({text = "222222222222", size = 25, align = ui.TEXT_ALIGN_CENTER})
        ...
        [3] = function()
            ...
            -- local label = ui.newTTFLabel({text = "333333333333333", size = 25, align = ui.TEXT_ALIGN_CENTER})
...
function ExpressionPanel:onExprTabChanged(selectedIdx)
    ...
    local getBottomStrByRoomType = {
        ...
        [consts.ROOM_TYPE.MATCH] = function()
            ...
            -- else
            --     matchData  = nk.MatchConfig:getMatchDataById(matchType)

            -- end
             

            -- dump(matchData, "matchData :=============")
            exprUseCost = matchData.exprCost or 0 -- 等待PHP的配置！
            -- exprUseCost = 80000
            -- 根据MatchType 取得付费表情费用
