...
function ChatMsgHistoryListItem:ctor()
    ...
    self.label_ = ui.newTTFLabel({text="", size=20, color=cc.c3b(0x72, 0x7f, 0x89),dimensions=cc.size(ChatMsgHistoryListItem.WIDTH, 0), align=ui.TEXT_ALIGN_LEFT})
...
function ChatMsgHistoryListItem:onDataSet(dataChanged, data)
    if dataChanged then
        ...
        if data.mtype == 1 then
            -- 大喇叭
            self.label_:setColor(cc.c3b(0xe0, 0x86, 0x1a))
        elseif data.mtype == 2 then
            -- 普通聊天消息
            self.label_:setColor(cc.c3b(0x72, 0x7f, 0x89))
        end
