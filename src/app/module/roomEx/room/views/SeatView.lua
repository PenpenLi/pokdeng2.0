...
function SeatView:ctor(ctx, seatId)
    ...
    --2 桌子背景
    --self.background_ = display.newScale9Sprite("#room_seat_bg.png", 0, 0, CCSize(SeatView.WIDTH, SeatView.HEIGHT)):addTo(self, 2)
    ...
    if appconfig.LANG == "vn" then
        self.state_ = ui.newTTFLabel({text = "Seat" .. seatId, size = 19, align = ui.TEXT_ALIGN_CENTER, color=cc.c3b(0xff, 0xd1, 0x0)})
    else
        self.state_ = ui.newTTFLabel({text = "Seat" .. seatId, size = 19, align = ui.TEXT_ALIGN_CENTER, color=cc.c3b(0xff, 0xff, 0xff)})
    end
    ...
    self.chips_ = ui.newTTFLabel({text = "", size = 19, align = ui.TEXT_ALIGN_CENTER, color=cc.c3b(0xff, 0xff, 0xff)})
        :pos(0, -66)
        :addTo(self, 7, 7)
    ...
    --10 牌型背景
    self.cardTypeBackground_ = display.newScale9Sprite("#room_seat_card_type_light_bg.png", 0, 0, CCSize(118,32)):addTo(self, 10, 10):hide()
...
function SeatView:showHandCardsAnimation()
    local sequence = transition.sequence({
        ...
        CCMoveTo:create(0.35, cc.p(240, 165)),
...
function SeatView:playAutoBuyinAnimation(buyinChips)
    ...
    local buyInLabelSequence = transition.sequence({
            spawn({
                ...
                CCMoveTo:create(1, cc.p(0, SeatView.HEIGHT/2 - buyInBgSize.height/2 - buyInLabelPaddding)),
            }),
            cc.CallFunc:create(function()
...
function SeatView:updateAnte(money)
