local DealerAdvanceNotice = class("DealerAdvanceNotice", function() return display.newNode()end)
local BG_WIDTH = 275
local BG_HEIGHT = 155
function DealerAdvanceNotice:ctor()
...
function DealerAdvanceNotice:createNode_()
    ...
    -- self.rect_= display.newScale9Sprite("#common_roundRect_grey.png", 0, 0, cc.size(BG_WIDTH, BG_HEIGHT))
    -- :addTo(self.bg_)
    self.isShow = false
...
function DealerAdvanceNotice:hideNotice()
...
--外部调用该方法
function DealerAdvanceNotice:addMsg(data)
	table.insert(self.dataPool_,data)
	self:showMsg()
end
--强制停止
function DealerAdvanceNotice:stop()
    self:stopAction(self.hideAction_)
...
function DealerAdvanceNotice:update()
...
function DealerAdvanceNotice:showMsg()
...
function DealerAdvanceNotice:onCleanup()
	self:stopAction(self.action_)
    self:stopAction(self.hideAction_)
end

return DealerAdvanceNotice