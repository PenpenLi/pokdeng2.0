...
local RaiseSlider = import(".views.RaiseSliderEx")
...
local OperationManagerEx = class("OperationManagerEx")
...
function OperationManagerEx:ctor()
    self.schedulerPool_ = bm.SchedulerPool.new()
end

function OperationManagerEx:createNodes()
    --聊天按钮
...
function OperationManagerEx:setLatestChatMsg(msg)
...
function OperationManagerEx:startLoading()
    self.trackBar_:show()
...
function OperationManagerEx:stopLoading()
    self.schedulerPool_:clear(self.startLoadingId_)
    self.startLoadingId_ = nil
    self.trackBar_:hide()
    self.barBackground_:hide()
...
function OperationManagerEx:dispose()
...
function OperationManagerEx:showOperationButtons(animation)
...
function OperationManagerEx:hideOperationButtons(animation)
    self.oprNode_:stopAllActions()
    self.extOptView_:hide()
    if animation then
...
function OperationManagerEx:showExtOperationView(animation)
    self.oprNode_:hide()
    self.tipsView_:hide()
...
function OperationManagerEx:showHandcardCallback_()
    self.schedulerPool_:clear(self.showOptSchedulerId)
    self.showOptSchedulerId = nil
    self.extOptView_:hide()
    self.oprNode_:show()
...
function OperationManagerEx:blockOperationButtons()
    self:disabledStatus_()
end

function OperationManagerEx:resetAutoOperationStatus()
    self.checkGroup_:onChecked(nil):uncheck()
    self.autoAction_ = nil
end

function OperationManagerEx:updateOperationStatus()
    self.schedulerPool_:clear(self.showOptSchedulerId)
    self.showOptSchedulerId = nil
    self.extOptView_:hide()
    self.oprNode_:show()
...
function OperationManagerEx:setSliderStatus(minRaiseChips, maxRaiseChips)
    local selfSeatData = self.model:selfSeatData()
    self.raiseSlider_:setValueRange(minRaiseChips, maxRaiseChips)
end

--无法操作的状态
function OperationManagerEx:disabledStatus_()
    self.chatNode_:show()
    self:hideOperationButtons(false)
    self.raiseSlider_:hidePanel()
end

function OperationManagerEx:getQuickCallValue_(minBet)
    if self.bet_ == 0 then
        self.bet_ = minBet
    end
    ...
    if self.bet_ == minBet then
        self._quickBetLeftValue = checkint(self.bet_ * 2)
        self._quickBetMidValue = checkint(self.bet_ * 5)
        self._quickBetRightValue = checkint(self.bet_ * 10)
    elseif self.bet_ <= minBet * 2 then
        self._quickBetLeftValue = checkint(minBet)
        self._quickBetMidValue = checkint(self.bet_ * 2)
        self._quickBetRightValue = checkint(self.bet_ * 5)
    else
        self._quickBetLeftValue = checkint(minBet)
        self._quickBetMidValue = math.ceil(self.bet_ * 0.5)
        self._quickBetRightValue = checkint(self.bet_ * 2)
    end
    return {self._quickBetLeftValue, self._quickBetMidValue, self._quickBetRightValue}
end

--下注
function OperationManagerEx:selfCanRaiseStatus_(minRaiseChips, maxRaiseChips)

    -- local myAnte = maxRaiseChips
    -- local miniBet = self.model.roomInfo.miniBet or 0
    -- local highBet = self.model.roomInfo.highBet or 0
    -- local betPro = self.model.roomInfo.betPro or 1

    


    -- if checkint(myAnte/betPro) >= highBet then
    --     maxRaiseChips = highBet
    -- elseif checkint(myAnte/betPro) < highBet and checkint(myAnte/betPro)> miniBet then
    --     maxRaiseChips = checkint(myAnte/betPro)
    -- elseif checkint(myAnte/betPro)<= miniBet and myAnte >=  miniBet then
    --     maxRaiseChips = miniBet
    -- else
    --     maxRaiseChips = myAnte
    -- end
    maxRaiseChips = checkint(maxRaiseChips)

   -- dump("myAnte:"..myAnte.."  miniBet:"..miniBet.."  highBet:"..highBet.." betPro:"..betPro.." maxRaiseChips:"..maxRaiseChips,"dddddd")
...
--要牌
function OperationManagerEx:getPokerStatus_()
    self:showOperationButtons(false)
    self.chatNode_:show()
...
function OperationManagerEx:getPokerForceStatus_()
    self:showOperationButtons(false)
    self.chatNode_:show()
    self.oprBtn1_:setLabel(LB_GET_POKER):setEnabled(true):setCheckMode(false):onTouch(handler(self, self.getPokerClickHandler))
...
-- 将要要牌
function OperationManagerEx:willGetPokerStatus_()
    self:showOperationButtons(false)
    self.chatNode_:show()
...
function OperationManagerEx:quickCall1Handler(evt)
...
function OperationManagerEx:quickCall2Handler(evt)
    if evt == bm.TouchHelper.CLICK then
...
function OperationManagerEx:quickCall3Handler(evt)
    if evt == bm.TouchHelper.CLICK then
...
function OperationManagerEx:getPokerClickHandler(evt)
    if evt == bm.TouchHelper.CLICK then
...
function OperationManagerEx:notGetPokerClickHandler(evt)
    if evt == bm.TouchHelper.CLICK then
...
--最小下注
function OperationManagerEx:callMiniClickHandler(evt)
    -- if evt == bm.TouchHelper.CLICK then
    ...
    if evt == bm.TouchHelper.CLICK then
        ...
        if self.bet_> self.maxBet_ then
            self.bet_ = self.maxBet_
        end
...
-- 下注
function OperationManagerEx:setBet_(bet)
    self.bet_ = bet
    nk.server:setBet(bet)
end

function OperationManagerEx:raiseRangeClickHandler(evt)
    if evt == bm.TouchHelper.CLICK then
...
-- 勾选了自动看牌跟注等，在这里自动发包
function OperationManagerEx:applyAutoOperation_()
...
function OperationManagerEx:onRaiseSliderButtonClicked_(tag)
    ...
    local totalChipsInTable = self.maxBet_
    if tag == 1 then
        self:setBet_(totalChipsInTable)
    elseif tag == 2 then
        self:setBet_(checkint(totalChipsInTable * 0.75))
    elseif tag == 3 then
        self:setBet_(checkint(totalChipsInTable * 0.5))
    elseif tag == 4 then
        self:setBet_(checkint(totalChipsInTable * 1/3))
    elseif tag == 5 then
        self:setBet_(checkint(self.raiseSlider_:getValue()))
    end
    self.raiseSlider_:hidePanel()
    self:disabledStatus_()
end

function OperationManagerEx:onBackgroundClicked()
    self.raiseSlider_:hidePanel()
end

return OperationManagerEx