...
local RoomModel = import(".model.GrabRoomModel")
local SeatManager = import(".GrabSeatManager")
local DealerManager = import(".GrabDealerManager")
local DealCardManager = import(".GrabDealCardManager")
local LampManager = import(".GrabLampManager")
local ChipManager = import(".GrabChipManager")
local AnimManager = import(".GrabAnimManager")
local OperationManager = import(".GrabOperationManager")
local BetChipView = import(".views.GrabBetChipView")
...
local GrabRoomController = class("GrabRoomController")
local logger = bm.Logger.new("GrabRoomController")

GrabRoomController.EVT_PACKET_RECEIVED = nk.server.EVT_PACKET_RECEIVED

local PACKET_PROC_FRAME_INTERVAL = 2

function GrabRoomController:ctor(scene)
    cc(self):addComponent("components.behavior.EventProtocol"):exportMethods()

    local ctx = {}
    ctx.GrabRoomController = self
    ctx.scene = scene
    ...
    self.func_ = {
        [P.SVR_DOUBLE_LOGIN] = handler(self, GrabRoomController.SVR_DOUBLE_LOGIN),
        [P.SVR_LOGIN_OK] = handler(self, GrabRoomController.SVR_LOGIN_OK),
        [P.SVR_LOGIN_ROOM_OK] = handler(self, GrabRoomController.SVR_LOGIN_ROOM_OK),
        [P.SVR_LOGIN_ROOM_FAIL] = handler(self, GrabRoomController.SVR_LOGIN_ROOM_FAIL),
        [P.SVR_LOGOUT_ROOM_OK] = handler(self, GrabRoomController.SVR_LOGOUT_ROOM_OK),
        [P.SVR_SEAT_DOWN] = handler(self, GrabRoomController.SVR_SEAT_DOWN),
        [P.SVR_STAND_UP] = handler(self, GrabRoomController.SVR_STAND_UP), 
        [P.SVR_OTHER_CARD] = handler(self, GrabRoomController.SVR_OTHER_CARD), 
        [P.SVR_SET_BET] = handler(self, GrabRoomController.SVR_SET_BET), 
        [P.SVR_MSG] = handler(self, GrabRoomController.SVR_MSG),
        [P.SVR_DEAL] = handler(self, GrabRoomController.SVR_DEAL),
        [P.SVR_LOGIN_ROOM] = handler(self, GrabRoomController.SVR_LOGIN_ROOM),
        [P.SVR_LOGOUT_ROOM] = handler(self, GrabRoomController.SVR_LOGOUT_ROOM),
        [P.SVR_SELF_SEAT_DOWN_OK] = handler(self, GrabRoomController.SVR_SELF_SEAT_DOWN_OK), 
        [P.SVR_OTHER_STAND_UP] = handler(self, GrabRoomController.SVR_OTHER_STAND_UP),
        [P.SVR_OTHER_OFFLINE] = handler(self, GrabRoomController.SVR_OTHER_OFFLINE),
        [P.SVR_GAME_START] = handler(self, GrabRoomController.SVR_GAME_START),
        [P.SVR_GAME_OVER] = handler(self, GrabRoomController.SVR_GAME_OVER),
        [P.SVR_BET] = handler(self, GrabRoomController.SVR_BET),
        [P.SVR_CAN_OTHER_CARD] = handler(self, GrabRoomController.SVR_CAN_OTHER_CARD),
        [P.SVR_OTHER_OTHER_CARD] = handler(self, GrabRoomController.SVR_OTHER_OTHER_CARD),
        [P.SVR_SHOW_CARD] = handler(self, GrabRoomController.SVR_SHOW_CARD),
        [P.SVR_CARD_NUM] = handler(self, GrabRoomController.SVR_CARD_NUM),
        [P.SVR_ROOM_BROADCAST] = handler(self,GrabRoomController.SVR_ROOM_BROADCAST),
        [P.SVR_SEARCH_PERSONAL_ROOM] = handler(self,GrabRoomController.SVR_SEARCH_PERSONAL_ROOM),
        [P.SVR_COMMON_BROADCAST] = handler(self,GrabRoomController.SVR_COMMON_BROADCAST),
        [P.SVR_LOGIN_GRAB_ROOM_OK] = handler(self, GrabRoomController.SVR_LOGIN_ROOM_OK),
        [P.SVR_GRAB_ROOM_GAME_START] = handler(self,GrabRoomController.SVR_GRAB_ROOM_GAME_START),
        [P.SVR_REQUEST_GRAB_DEALER_RESULT] = handler(self,GrabRoomController.SVR_REQUEST_GRAB_DEALER_RESULT),
        [P.SVR_BROADCAST_RE_GRAB_DEALER_USER] = handler(self,GrabRoomController.SVR_BROADCAST_RE_GRAB_DEALER_USER),
        [P.SVR_BROADCAST_GRAB_PLAYER]   = handler(self,GrabRoomController.SVR_BROADCAST_GRAB_PLAYER),
        [P.SVR_BROADCAST_CAN_GRAB_DEALER] = handler(self,GrabRoomController.SVR_BROADCAST_CAN_GRAB_DEALER),
        [P.SVR_EXIT_DELAER_FAIL] = handler(self,GrabRoomController.SVR_EXIT_DELAER_FAIL),
        [P.SVR_WARRING_DEALER_ADD_COIN] = handler(self,GrabRoomController.SVR_WARRING_DEALER_ADD_COIN),
        [P.SVR_PLAYER_ADD_COIN_RESULT] = handler(self,GrabRoomController.SVR_PLAYER_ADD_COIN_RESULT),
        [P.SVR_DEALER_SITDOWN_FAIL] = handler(self,GrabRoomController.SVR_DEALER_SITDOWN_FAIL),
        [P.SVR_SUONA_BROADCAST] = handler(self, GrabRoomController.SVR_SUONA_BROADCAST_RECV),  -- 房间内小喇叭广播
        [P.SVR_PUSH_MATCH_ROOM] = handler(self, GrabRoomController.SVR_PUSH_MATCH_ROOM),
        [P.SVR_LOGIN_OK_RE_CONECT] = handler(self, GrabRoomController.SVR_LOGIN_OK_RE_CONECT),
    }
...
function GrabRoomController:dispose()
...
function GrabRoomController:createNodes()
...
function GrabRoomController:onLoginRoomFail_(evt)
...
function GrabRoomController:onServerFail_(evt)    
    --连接失败
...
function GrabRoomController:showErrorByDialog_(msg)
...
function GrabRoomController:onServerStop_(evt)
    if self.roomLoading_ then
        self.roomLoading_:removeFromParent()
        self.roomLoading_ = nil
    end
...
function GrabRoomController:onConnected_(evt)
    self.packetCache_ = {}
end

function GrabRoomController:onEnterFrame_(dt)
...
function GrabRoomController:onPacketReceived_(evt)
    print("GrabRoomController:onPacketReceived_")
    table.insert(self.packetCache_, evt.packet)
end

function GrabRoomController:SVR_DOUBLE_LOGIN(pack)
...
function GrabRoomController:SVR_LOGIN_OK(pack)
    -- local data = pack
    -- if data.tid > 0 then
    --     -- 重连房间
    --     self.tid__ = data.tid
...
function GrabRoomController:SVR_LOGIN_OK_RE_CONECT(pack)
    -- body
    if pack.tid <= 0 then
        self.scene:doBackToHall()
        return
...
function GrabRoomController:SVR_LOGIN_ROOM_OK(pack)   
    ...
    local showGrabBtn = true
     for i = 0, 9 do
        ...
        if player and player.seatId == 9 then
            showGrabBtn = false
        end
    end
    if showGrabBtn == true then
        ...
        self.scene.roomBaseView_:setSendDealerChipState(true)
    else
...
function GrabRoomController:SVR_LOGIN_ROOM_FAIL(pack)
    dump(pack,"SVR_LOGIN_ROOM_FAIL")
end

function GrabRoomController:SVR_LOGOUT_ROOM_OK(pack)
    --self.scene:doBackToHall()
...
function GrabRoomController:SVR_SEAT_DOWN(pack)
    ...
    if seatId == 9 then -- 强制更新
        ...
        self.scene.roomBaseView_:setSendDealerChipState(false)
    end
    --自己是庄家，开启庄家位补币按钮
    if model:selfSeatId() == seatId and seatId == 9 then
        self.scene:showDealerAddCoinBtn(true)
...
function GrabRoomController:SVR_SET_BET(pack)
end

function GrabRoomController:SVR_MSG(pack)
end

function GrabRoomController:SVR_DEAL(pack)
    local ctx = self.ctx
    local model = self.model
...
function GrabRoomController:SVR_LOGIN_ROOM(pack)

end

function GrabRoomController:SVR_LOGOUT_ROOM(pack)    
end

function GrabRoomController:SVR_SELF_SEAT_DOWN_OK(pack)   
    if pack.ret ~= 0 then
        --坐下失败
...
function GrabRoomController:SVR_STAND_UP(pack)
    if pack.ret ~= 0 then
...
function GrabRoomController:SVR_OTHER_STAND_UP(pack)
    local ctx = self.ctx
    local model = self.model
...
function GrabRoomController:SVR_OTHER_OFFLINE(pack)    
end

function GrabRoomController:SVR_GAME_START(pack)
    local ctx = self.ctx
    local model = self.model
...
function GrabRoomController:SVR_BROADCAST_RE_GRAB_DEALER_USER(pack)
    self.scene:setGrabLeftTime(tonumber(pack.lefttime))
end
function GrabRoomController:SVR_BROADCAST_GRAB_PLAYER(pack)
    local model = self.model
    model:processGrabAdvance(pack)
    self.scene:setGrabPlayerData(pack)
end
function GrabRoomController:SVR_BROADCAST_CAN_GRAB_DEALER(pack)
    local ctx = self.ctx
    local model = self.model
    --庄家位不参与抢庄
...
function GrabRoomController:SVR_EXIT_DELAER_FAIL(pack)
    nk.TopTipManager:showTopTip(T("玩牌至少五局"))
end
function GrabRoomController:SVR_WARRING_DEALER_ADD_COIN(pack)
    local ctx = self.ctx
    local model = self.model
...
function GrabRoomController:SVR_PLAYER_ADD_COIN_RESULT(pack)
...
function GrabRoomController:SVR_DEALER_SITDOWN_FAIL(pack)
    nk.TopTipManager:showTopTip(T("上庄失败"))
end
function GrabRoomController:SVR_GRAB_ROOM_GAME_START(pack)
    local ctx = self.ctx
    local model = self.model
...
--server回复抢庄结果
function GrabRoomController:SVR_REQUEST_GRAB_DEALER_RESULT(pack)
...
function GrabRoomController:turnTo_(seatId)
    local ctx = self.ctx
    local model = self.model
    if model:selfSeatId() == seatId then
        nk.SoundManager:playSound(nk.SoundManager.NOTICE)
...
--下注
function GrabRoomController:SVR_BET(pack)
    local ctx = self.ctx
    local model = self.model
    --下注成功
...
function GrabRoomController:SVR_CAN_OTHER_CARD(pack)
    local model = self.model
    local ctx = self.ctx
...
function GrabRoomController:SVR_OTHER_OTHER_CARD(pack)
    local model = self.model
    local ctx = self.ctx
    --发第三张牌
...
function GrabRoomController:SVR_OTHER_CARD(pack)
    local ctx = self.ctx
    local model = self.model
...
-- 亮牌
function GrabRoomController:SVR_SHOW_CARD(pack)
    local model = self.model
...
-- 广播发牌 
function GrabRoomController:SVR_CARD_NUM(pack)
    local ctx = self.ctx
    local model = self.model
...
function GrabRoomController:SVR_SEARCH_PERSONAL_ROOM(pack)
    self.scene:onSearchPersonalRoom(pack)
end


--发送房间内广播
function GrabRoomController:SVR_ROOM_BROADCAST(pack)
    -- dump(pack,"GrabRoomController:SVR_ROOM_BROADCAST==========发送房间内广播")
...
function GrabRoomController:SVR_PUSH_MATCH_ROOM(pack)
    -- body
...
function GrabRoomController:SVR_SUONA_BROADCAST_RECV(pack)
    -- body
    -- dump(pack, "GrabRoomController.SVR_SUONA_BROADCAST_RECV :=================")
...
function GrabRoomController:SVR_COMMON_BROADCAST(pack)
...
function GrabRoomController:SVR_GAME_OVER(pack)
    local ctx = self.ctx
    local model = self.model
    self.gameSchedulerPool:clearAll()
...
function GrabRoomController:processUpgrade(delay)
    ...
    if nowExp > oldExp then
        ...
        if nk.userData.nextRwdLevel and nk.userData.nextRwdLevel ~= 0 then
            display.addSpriteFrames("upgrade_texture.plist", "upgrade_texture.png", function(fileName, imgName)
...
function GrabRoomController:processBestMaxMoney()
...
function GrabRoomController:processBankrupt(delay,showView)
                        ...
                        self.schedulerPool:delayCall(function()
                            ...
                            if nk.userData.bankruptcyGrant.bankruptcyTimes == 0 then
                                ...
                                if isThirdPayOpen then
                                    --todo
                                    if nk.OnOff:check("firstchargeFavGray") and isFirstCharge then
                                        --todo
                                        ...
                                        if nk.OnOff:check("rechargeFavGray") and isPay and rechargeFavAccess then
                                            --todo
...
function GrabRoomController:processPacket_(pack)
    if self.func_[pack.cmd] then
        self.func_[pack.cmd](pack)
    end   
end

function GrabRoomController:applyAutoSitDown()
    if not self.model:isSelfInSeat() then
        ...
        if emptySeatId then
            ...
            if isAutoSit or nk.server:isPlayNow() then
                ...
                if myMoney >= self.model.roomInfo.minBuyIn then
                    ...
                    local message = bm.LangUtil.getText("COMMON", "NOT_ENOUGH_MONEY_TO_PLAY_NOW_MSG", self.model.roomInfo.minBuyIn)
                    if isCash == 1 then
                        message =bm.LangUtil.getText("COMMON", "NOT_ENOUGH_CASH_TO_PLAY_NOW_MSG", self.model.roomInfo.minBuyIn)
                    end
...
function GrabRoomController:updateChangeRoomButtonMode()
...
function GrabRoomController:onBoroadCastMsgJump_()
    -- body
...
function GrabRoomController:reset()
    self.hasReset_ = true
...
function GrabRoomController:checkMoneyChange(money)
...
function GrabRoomController:bindDataObservers_()
