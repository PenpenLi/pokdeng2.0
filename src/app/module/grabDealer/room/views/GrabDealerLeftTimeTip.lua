local GrabDealerLeftTimeTip = class("GrabDealerLeftTimeTip", function() return display.newNode() end)
local GrabDealerBuyInPopup = import("app.module.grabDealer.room.views.GrabDealerBuyInPopup")
function GrabDealerLeftTimeTip:ctor(callback)
...
function GrabDealerLeftTimeTip:createNode_()
    ...
    local ButtonSize = cc.size(131,62)
     cc.ui.UIPushButton.new({normal = "#common_modTransparent.png", pressed = "#common_roundRect_grey.png"}, {scale9 = true})
       :align(display.LEFT_CENTER)
...
function GrabDealerLeftTimeTip:showTime(lefttime,door,iscash)
...
function GrabDealerLeftTimeTip:onGrabDealer()
    ...
    if myMoney - self.door_ < 0 then
        if self.iscash_ == 1 then
           --nk.TopTipManager:showTopTip(T("现金币不足"))
            nk.TopTipManager:showTopTip(bm.LangUtil.getText("GRAB_DEALER", "GRAB_ROOM_NOT_CASH",self.door_))
        else
            --nk.TopTipManager:showTopTip(T("金币不足"))
            nk.TopTipManager:showTopTip(bm.LangUtil.getText("GRAB_DEALER", "GRAB_ROOM_NOT_MONEY2",self.door_))
        end
        return
    end
...
function GrabDealerLeftTimeTip:onCleanup()
	self:stopAction(self.action_)
end

return GrabDealerLeftTimeTip