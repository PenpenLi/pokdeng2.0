...
local GrabNormalRoomBaseView = class("GrabNormalRoomBaseView",function()
...
function GrabNormalRoomBaseView:ctor(ctx)
	self.ctx = ctx
    ...
    --roomTableLeft:setAnchorPoint(ccp(1, 0.5))
    roomTableLeft:pos(display.cx, RoomViewPosition.SeatPosition[1].y - roomTableLeft:getContentSize().height * 0.5):addTo(batchNode)
    -- local roomTableLeft = display.newSprite("#room_table.png")
    ...
    self.roomInfo_ = ui.newTTFLabel({size=18, text="", color=cc.c3b(0x71, 0x4f, 0x1a)})
        :pos(display.cx, display.cy + 156)
        :addTo(self)
    local marginLeft = 32
    local marginTop = -30
    ...
    if  not isFirstSendDearlerChip then
        ...
        self.sendDealerChipIcon:addTo(self.toDealerSendChip_)
...
function GrabNormalRoomBaseView:addDataObservers()
    -- body
...
function GrabNormalRoomBaseView:updataCards(data)
    local result = data[2]
...
function GrabNormalRoomBaseView:setStoreDiscount(discount)
...
function GrabNormalRoomBaseView:setRoomInfoText(roomInfo)
    
    --local roomFiled = bm.LangUtil.getText("HALL", "ROOM_LEVEL_TEXT")[math.floor((roomInfo.roomType - 101) / 6) + 1]
    local roomFiled = T("抢庄房")
    if roomInfo and roomInfo.rmType == consts.ROOM_TYPE_EX.GRAB_CASH then
        roomFiled = T("现金币场")
    end
...
function GrabNormalRoomBaseView:onGrabInviteClick_()  
    if self. ctx.scene["onGrabInviteClick_"] then
        self.ctx.scene:onGrabInviteClick_()
    end
end
function GrabNormalRoomBaseView:setMatchInviteButtonVisible(visible)
    if not visible then
...
function GrabNormalRoomBaseView:setChangeRoomButtonMode(mode)
    if mode == 1 then
...
function GrabNormalRoomBaseView:setSendDealerChipState(state)
    -- body
    -- if state then
    --     --todo
    --     self.toDealerSendChip_:show()
    -- else
    --     self.toDealerSendChip_:hide()
    -- end
end

function GrabNormalRoomBaseView:refreshFirstChargeFavEntryUi(isOpen)
    -- body
    if not isOpen then
        --todo
...
function GrabNormalRoomBaseView:refreshAlmRechargeFavEntryUi(isOn)
    -- body
    if not isOn then
        --todo
...
function GrabNormalRoomBaseView:onStandupClick_()
...
function GrabNormalRoomBaseView:onShopClick_()
    if self.ctx.scene["onShopClick_"] then
		self.ctx.scene:onShopClick_()
	end
end

function GrabNormalRoomBaseView:_onChargeFavCallBack()
    -- body
...
function GrabNormalRoomBaseView:onAlmRechargeFavCallBack_(evt)
    -- body
...
function GrabNormalRoomBaseView:onExpClick_()
...
function GrabNormalRoomBaseView:onCardTypeClick_()
    if self.ctx.scene["onCardTypeClick_"] then
        self.ctx.scene:onCardTypeClick_()
    end
end
function GrabNormalRoomBaseView:onRoomChatClick_()
    nk.SoundManager:playSound(nk.SoundManager.CLICK_BUTTON)
...
function GrabNormalRoomBaseView:onMenuClick_()
...
function GrabNormalRoomBaseView:onChangeRoomClick_()
...
function GrabNormalRoomBaseView:toDelearSendChipHandler()
	if self.ctx.scene["toDelearSendChipHandler"] then
		self.ctx.scene:toDelearSendChipHandler()
	end
end

function GrabNormalRoomBaseView:onCleanup()
    -- body
