...
local GrabDealerBuyInPopup = class("GrabDealerBuyInPopup", nk.ui.Panel)
...
function GrabDealerBuyInPopup:ctor(param)
    GrabDealerBuyInPopup.super.ctor(self, {WIDTH, HEIGHT})
    ...
    self.middlePercent_ =(self.max_  - self.min_)* 0.5 / self.range_
    ...
    self.autoBuyInChkboxLabel_ = display.newTTFLabel({size=18, color=cc.c3b(0xD3, 0xEA, 0xFF), text=T("抢庄金额越高，成功率越大")}):addTo(self)
...
function GrabDealerBuyInPopup:onThumbTouch_(evt)
...
function GrabDealerBuyInPopup:onEnterFrame_()
    if self.posPercent_ <= self.myMoneyPercent_ then
...
function GrabDealerBuyInPopup:onSliderPercentValueChanged_(newVal, forceUpdate, needSound)
...
function GrabDealerBuyInPopup:showPanel()
    self:showPanel_(true, true, true, true)
end

function GrabDealerBuyInPopup:hidePanel()
    self:hidePanel_()
end

function GrabDealerBuyInPopup:onEnter()
...
function GrabDealerBuyInPopup:onExit()
...
function GrabDealerBuyInPopup:onMoneyChanged_(money)
