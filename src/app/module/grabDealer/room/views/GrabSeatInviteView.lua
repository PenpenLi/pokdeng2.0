...
local GrabSeatInviteView = class("GrabSeatInviteView", function() 
    return display.newNode()
end)

GrabSeatInviteView.CLICKED = "GrabSeatInviteView.CLICKED"
GrabSeatInviteView.WIDTH = 108
GrabSeatInviteView.HEIGHT = 164

function GrabSeatInviteView:ctor(ctx)
	self.ctx_ = ctx
...
function GrabSeatInviteView:createNode_()
	self.background_ = display.newScale9Sprite("#room_seat_bg.png", 0, 0, cc.size(GrabSeatInviteView.WIDTH, GrabSeatInviteView.HEIGHT)):addTo(self, 2)
...
function GrabSeatInviteView:setViewInfo_(data)
...
function GrabSeatInviteView:onGetData_(success, friendData, filterStr)
    if success then
...
function GrabSeatInviteView:changeFriend() 
    local index = checkint(math.random(#self.friendData_))
    self.tempFriend = self.friendData_[index];
    self:setViewInfo_(self.tempFriend)
end

function GrabSeatInviteView:onInviteClick_()
	local toIds = ""
    local names = ""
	local toIdArr = {}
    local nameArr = {}
...
function GrabSeatInviteView:onReciveData(evt)
...
function GrabSeatInviteView:runTime()
    self:stopAction(self.actions_)
    self.actions_ = self:schedule(function ()
        self:changeFriend()
        self:runTime()
...
function GrabSeatInviteView:onCleanup()
	bm.EventCenter:removeEventListener(nk.eventNames.UPDATE_SEAT_INVITE_VIEW)
end

function GrabSeatInviteView:stopAll()
    self.inviteSeatId = nil
    self:hide()
    self.isShow = false;
    self:stopAction(self.actions_)
