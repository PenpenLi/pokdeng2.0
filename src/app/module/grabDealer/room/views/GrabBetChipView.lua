...
local GrabBetChipView = class("GrabBetChipView")
local RoomViewPosition = import(".GrabRoomViewPosition")
local SP = RoomViewPosition.SeatPosition
local BP = RoomViewPosition.BetPosition
local PP = RoomViewPosition.PotPosition

GrabBetChipView.MOVE_FROM_SEAT_DURATION = 0.4
GrabBetChipView.MOVE_TO_POT_DURATION = 0.5
local MOVE_DELAY_DURATION = 0.075

local GAP_WITH_CHIPS = 4

function GrabBetChipView:ctor(parent, manager, seatId)
    self.parent_ = parent
...
function GrabBetChipView:rotate(positionId)
    if self.betGrabChipData_ then
        for i, GrabChipData in ipairs(self.betGrabChipData_) do
            GrabChipData:getSprite():pos(BP[positionId].x - 38, BP[positionId].y + (i - 1) * GAP_WITH_CHIPS)
        end
        return self
    end
end
...
-- 创建筹码堆
function GrabBetChipView:createChipStack()
    if self.betGrabChipData_ then
...
-- 重置筹码堆
function GrabBetChipView:resetChipStack(betChips)
...
-- 获取某一轮当前下注筹码
function GrabBetChipView:getBetTotalChips()
    return self.betTotalChips_ or 0
end

-- 从座位飞出
function GrabBetChipView:moveFromSeat(betNeedChips, betTotalChips)
        ...
        for i, GrabChipData in ipairs(self.movingGrabChipData_) do
            ...
            if i < #self.movingGrabChipData_ then
                transition.execute(
                    sp, 
                    cc.MoveTo:create(
                        GrabBetChipView.MOVE_FROM_SEAT_DURATION, 
...
function GrabBetChipView:moveFromSeatComplete_()
    -- 有上一次的下注筹码数据，则回收
    if self.lastBetGrabChipData_ then
        if self.lastBetGrabChipData_ == self.potGrabChipData_ then
            self.potGrabChipData_ = nil
...
function GrabBetChipView:setPotGrabChipData()
...
-- 移动至奖池
function GrabBetChipView:moveToPot()
        ...
        for i, GrabChipData in ipairs(self.potGrabChipData_) do
            ...
            if i > 1 then
                transition.execute(
                    sp, 
                    cc.MoveTo:create(
                        GrabBetChipView.MOVE_TO_POT_DURATION, 
...
function GrabBetChipView:moveToPotComplete_()
    if self.potGrabChipData_ == self.betGrabChipData_ then
...
function GrabBetChipView:reset()
    -- 回收筹码数据
    if self.lastBetGrabChipData_ then
...
-- 清理
function GrabBetChipView:dispose()
    self:reset()
end

return GrabBetChipView