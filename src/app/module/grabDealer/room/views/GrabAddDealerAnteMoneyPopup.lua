...
local GrabAddDealerAnteMoneyPopup = class("GrabAddDealerAnteMoneyPopup", nk.ui.Panel)
...
function GrabAddDealerAnteMoneyPopup:ctor(param)
    GrabAddDealerAnteMoneyPopup.super.ctor(self, {WIDTH, HEIGHT})

    self.schedulerPool_ = bm.SchedulerPool.new()
    self:setNodeEventEnabled(true)
    self.param_ = param
    self.isCash_ = param.isCash
    if self.isCash_ == 1 then
        self.myMoney_ = nk.userData['match.point']
    else
        self.myMoney_ = nk.userData['aUser.money']
    end
    self.min_ = param.minBuyIn
    self.max_ = param.maxBuyIn
    self.range_ = self.max_ - self.min_
    self.step_ = math.ceil(self.range_ / 10)
    self.myMoneyPercent_ = ((nk.userData and self.myMoney_ or 0) - self.min_) / self.range_
    ...
    local curValueSize = self.curValueLabel_:getContentSize()
    self.cashicon1_ = display.newSprite("#cash_coin_icon.png")
    :addTo(self):pos(-curValueSize.width/2-20, self.curValueBg_:getPositionY())
    self.cashicon1_:setScale(.7)
    ...
    self.autoBuyInChkboxBg_ = display.newSprite("#room_buyin_checkbox_bg.png", 0, -90):addTo(self):hide()
    self.autoBuyInChkboxIcon_ = display.newSprite("#room_buyin_checkbox_icon.png"):addTo(self):hide()
    self.autoBuyInChkboxPressed_ = display.newScale9Sprite("#common_button_pressed_cover.png", 0, 0, cc.size(20, 20)):addTo(self):hide()
    self.autoBuyInChkboxLabel_ = display.newTTFLabel({size=18, color=cc.c3b(0xD3, 0xEA, 0xFF), text=T("追加金币越高，保庄成功率越大")}):addTo(self)
    ...
    cc.ui.UIPushButton.new("#common_modTransparent.png", {scale9=true}):setButtonSize(chkboxW + 16, chkboxH)
    ...
    self.buyInBtn_ = cc.ui.UIPushButton.new({normal="#common_green_btn_up.png", pressed="#common_green_btn_down.png", disabled="#common_btn_disabled.png"}, {scale9=true})
        :setButtonLabel(display.newTTFLabel({size=24, text=T("确定"), color=cc.c3b(0xcb, 0xe6, 0xff)}))
    ...
    self.buyBtn_ = cc.ui.UIPushButton.new({normal="#common_modTransparent.png", pressed="#common_button_pressed_cover.png"}, {scale9=true})
...
function GrabAddDealerAnteMoneyPopup:onEnterFrame_()
    if self.posPercent_ <= self.myMoneyPercent_ then
...
function GrabAddDealerAnteMoneyPopup:onSliderPercentValueChanged_(newVal, forceUpdate, needSound)
    if self.percentValue_ ~= newVal or forceUpdate then
        ...
        if moneyVal > (nk.userData and self.myMoney_ or 0) then
        ...
        local curValueSize = self.curValueLabel_:getContentSize()
        self.cashicon1_:setPositionX(-curValueSize.width/2-20)
...
function GrabAddDealerAnteMoneyPopup:showPanel()
    self:showPanel_(true, true, true, true)
end

function GrabAddDealerAnteMoneyPopup:hidePanel()
    self:hidePanel_()
end

function GrabAddDealerAnteMoneyPopup:onEnter()
    if not self.moneyChangeObserverId_ then
        if self.isCash_ == 1 then
            self.moneyChangeObserverId_ = bm.DataProxy:addPropertyObserver(nk.dataKeys.USER_DATA, "match.point", handler(self, self.onMoneyChanged_))
        else
...
function GrabAddDealerAnteMoneyPopup:onExit()
    if self.moneyChangeObserverId_ then
        ...
        bm.DataProxy:removePropertyObserver(nk.dataKeys.USER_DATA, "match.point", self.moneyChangeObserverId_)
...
function GrabAddDealerAnteMoneyPopup:onMoneyChanged_(money)
...
return GrabAddDealerAnteMoneyPopup