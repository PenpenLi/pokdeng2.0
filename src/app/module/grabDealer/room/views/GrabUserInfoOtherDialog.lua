...
local GrabUserInfoOtherDialog = class("GrabUserInfoOtherDialog", function() return nk.ui.Panel.new({WIDTH, HEIGHT}) end)

function GrabUserInfoOtherDialog:ctor(ctx)
    ...
    self.addFriendBtn_ = cc.ui.UIPushButton.new({
        ...
        :setButtonLabel(display.newTTFLabel({text=bm.LangUtil.getText("ROOM", "FOLLOW"), size=22, color=cc.c3b(0xFF, 0xFF, 0xFF)}))
    ...
    --UID
    self.uid_ = display.newTTFLabel({size=24, color=cc.c3b(0xce, 0xe8, 0xff)})
    ...
    -- 现金币图标
    local cashIcon = display.newSprite("#cash_coin_icon.png", LEFT + 164, TOP - 42 - 36 * 3):addTo(self)
    cashIcon:scale(.7)

    self.cash_ = display.newTTFLabel({text = "0", size = 24, color = cc.c3b(0xff, 0xd5, 0x31)})
    self.cash_:setAnchorPoint(display.ANCHOR_POINTS[display.CENTER_LEFT])
    self.cash_:pos(LEFT + 186, TOP - 42 - 36 * 3)
        :addTo(self)

    --排名
    self.ranking_ = display.newTTFLabel({size=24, color=cc.c3b(0xce, 0xe8, 0xff)})
    self.ranking_:setAnchorPoint(cc.p(0, 0.5))
    self.ranking_:pos(LEFT + 150, TOP - 42 - 36 * 4)
...
function GrabUserInfoOtherDialog:getFriendDataByUid(friendUid)
    -- body
...
function GrabUserInfoOtherDialog:loadFriendUidList()
...
function GrabUserInfoOtherDialog:show(data)
    self:setData(data)
    self:showPanel_(true, true, true, true)
end

function GrabUserInfoOtherDialog:hide()
...
function GrabUserInfoOtherDialog:setAddFriendStatus()
   ...
   -- dump(nk.userData.friendUidList,"nk.userData.friendUidList")
    if self.data_ and nk.userData.friendUidList then
        ...
        if not table.indexof(nk.userData.friendUidList, tostring(self.data_.uid)) then
            ...
            self.addFriendBtn_:setButtonLabelString(bm.LangUtil.getText("ROOM", "FOLLOW"))
            self.isAddFriend_ = true
        else
            ...
            if checkint(friendData.isFBfriend or 0) == 1 then
                ...
                self.addFriendBtn_:setButtonLabelString(bm.LangUtil.getText("ROOM", "UN_FOLLOW"))
                self.isAddFriend_ = false
            end
        end
    end
...
function GrabUserInfoOtherDialog:setData(data)
    self.data_ = data
    if data then
        -- dump(data, "GrabUserInfoOtherDialog:setData.data :================")
        ...
        self.reqGetUserInfoData_ = nk.http.getMemberInfo(data.userId or data.uid, function(retData)
            ...
            local userCashNum = retData.match.point or data.userInfo.cash or 0
            self.cash_:setString(bm.formatBigNumber(userCashNum))
            ...
            self.cash_:setString(bm.formatBigNumber(data.userInfo.cash or 0))
            if data.userInfo.rankMoney then
                if data.userInfo.rankMoney > 10000 then
                    self.ranking_:setString(bm.LangUtil.getText("ROOM", "INFO_RANKING", ">10,000"))
                else
...
function GrabUserInfoOtherDialog:setImage_(sprite)
...
function GrabUserInfoOtherDialog:imageLoadCallback_(success, sprite)
    if success then
...
function GrabUserInfoOtherDialog:onFriendClicked_(evt)
...
function GrabUserInfoOtherDialog:onAddFriendClicked_(evt)
...
function GrabUserInfoOtherDialog:onDelFriendClicked_(evt)
    ...
    self.addFriendBtn_:setButtonEnabled(false);
     self.deleteFriendRequestId_ = nk.http.deleteFriend(self.data_.uid,
        function (data)
            ...
            if idx then
                table.remove(nk.userData.friendUidList, tostring(idx))
             end
             self:setAddFriendStatus()
        end,
        function()
...
function GrabUserInfoOtherDialog:sendChipClicked_(chips)
    if self.ctx.model:isSelfInSeat() then
        local roomData = nk.getRoomDataByLevel(self.ctx.model.roomInfo.roomType)

        if not roomData or self.ctx.model.roomInfo.roomType == consts.ROOM_TYPE.PERSONAL_NORMAL then
            local roomInfo = self.ctx.model.roomInfo
            roomData = {}
            roomData.minBuyIn = roomInfo.minBuyIn
            roomData.maxBuyIn = roomInfo.maxBuyIn
        ...
        if (nk.userData["aUser.money"] -  chips ) > roomData.minBuyIn then
               nk.http.sendCoinForRoomEx(nk.userData.uid,self.data_.uid, chips,room_type,self.ctx.model.roomInfo.blind,
                   function (data)
                    -- dump(data,"senddata")
...
function GrabUserInfoOtherDialog:sendHddjClicked_(hddjId)
...
function GrabUserInfoOtherDialog:doSendHddj()
...
function GrabUserInfoOtherDialog:sendHddjAndHide_()
...
function GrabUserInfoOtherDialog:showHddjNotEnoughDialog_()
    nk.ui.Dialog.new({
...
function GrabUserInfoOtherDialog:onCleanup()
    if self.addFriendRequistId_ then
...
function GrabUserInfoOtherDialog:onExit()
    nk.ImageLoader:cancelJobByLoaderId(self.headImageLoaderId_)
...
return GrabUserInfoOtherDialog