...
--
local ChatMsgShortcutListItem = import(".GrabChatMsgShortcutListItem")
local ChatMsgHistoryListItem = import(".GrabChatMsgHistoryListItem")
local ChatTabPanel = import(".GrabChatTabPanel")
local GrabChatMsgPanel = class("GrabChatMsgPanel", ChatTabPanel)
...
function GrabChatMsgPanel:ctor()
    GrabChatMsgPanel.super.ctor(self, bm.LangUtil.getText("ROOM", "CHAT_TAB_SHORTCUT"), bm.LangUtil.getText("ROOM", "CHAT_TAB_HISTORY"))
    ...
    self.editBox_ = cc.ui.UIInput.new({
        ...
        size = cc.size(295, 58),
        x = - GrabChatMsgPanel.PAGE_WIDTH * 0.5 + 212,
    ...
    self.laSelectImage_ = cc.ui.UICheckBoxButton.new({off="#room-chat-pane-laba-background-up.png",on = "#room-chat-pane-laba-background-up.png"})
        ...
        :pos(- GrabChatMsgPanel.PAGE_WIDTH * 0.5 + 32, self.HEIGHT * 0.5 - 8 - 30 - 68)
    ...
    self.laBaNumberLabel_ = display.newTTFLabel({size=14, text="x0", color=cc.c3b(0x27, 0x90, 0xd5)})
        :pos(- GrabChatMsgPanel.PAGE_WIDTH * 0.5 + 55, self.HEIGHT * 0.5 - 8 - 52 - 68)
        :hide()
        :addTo(self)
    ...
    self.sendBtn_ = cc.ui.UIPushButton.new({normal="#common_green_btn_up.png", pressed="#common_green_btn_down.png", disabled="#common_btn_disabled.png"}, {scale9=true})
        ...
        :pos(GrabChatMsgPanel.PAGE_WIDTH * 0.5 - 58, self.editBox_:getPositionY() - 2)
...
function GrabChatMsgPanel:createShortcutPage_()
    local page = display.newNode()

    --快捷消息
    local listW, listH = GrabChatMsgPanel.PAGE_WIDTH, GrabChatMsgPanel.PAGE_HEIGHT - 68
...
function GrabChatMsgPanel:createHistoryPage_()
    local page = display.newNode()
    --快捷消息
    local listW, listH = GrabChatMsgPanel.PAGE_WIDTH, GrabChatMsgPanel.PAGE_HEIGHT - 68
...
function GrabChatMsgPanel:onEnter()
...
function GrabChatMsgPanel:onExit()
...
function GrabChatMsgPanel:historyChanged_(list)
    local mergedList = {}
...
function GrabChatMsgPanel:onEditBoxStateChange_(evt, editbox)
...
function GrabChatMsgPanel:onSendClicked_()
    if nk.userData.silenced == 1 then
        nk.TopTipManager:showTopTip(bm.LangUtil.getText("COMMON", "USER_SILENCED_MSG"))
        return
    end
...
function GrabChatMsgPanel:selectChangeListener(evt)
...
function GrabChatMsgPanel:laSelectImageHandler_()
   if  self.laBaNumber_ and self.laBaNumber_ <= 0 then
            nk.ui.Dialog.new({
            hasCloseButton = true,
            ...
            callback = function (type)
                ...
                elseif type == nk.ui.Dialog.SECOND_BTN_CLICK then
                    -- StorePopup.new(2, nil):showPanel()
