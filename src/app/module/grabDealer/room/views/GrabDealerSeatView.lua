...
local HandCard = import(".GrabHandCard")

local GrabDealerSeatView = class("GrabDealerSeatView", function() 
    return display.newNode()
end)

GrabDealerSeatView.CLICKED = "GrabDealerSeatView.CLICKED"
GrabDealerSeatView.WIDTH = 108
GrabDealerSeatView.HEIGHT = 164

function GrabDealerSeatView:ctor(ctx, seatId)
    self:retain()
    self.ctx = ctx
    cc(self):addComponent("components.behavior.EventProtocol"):exportMethods()
    self:setNodeEventEnabled(true)
    --2 桌子背景
    --self.background_ = display.newScale9Sprite("#room_seat_bg.png", 0, 0, cc.size(GrabDealerSeatView.WIDTH, GrabDealerSeatView.HEIGHT)):addTo(self, 2)
    ...
    self.nameBg_ = display.newSprite("#room_seat_name_bg.png"):addTo(self)
        :pos(0, -58)
    --6 状态文字
    ...
    if nk.config.GIFT_SHOP_ENABLED then
           self.giftImage_ = cc.ui.UIPushButton.new({normal = "#gift-icon-up.png",pressed = "#gift-icon-down.png"}):addTo(self, 10, 10):hide()
            :onButtonClicked(buttontHandler(self, self.openGiftPopUpHandler))
    end
    

    --9 winner动画
    self.winnerAnimBatch_ = display.newBatchNode("room_texture.png", 8):addTo(self, 9, 9):hide()
    --winner文字
...
function GrabDealerSeatView:playSitDownAnimation(onCompleteCallback)
...
function GrabDealerSeatView:playStandUpAnimation(onCompleteCallback)
...
function GrabDealerSeatView:standUpState_()
    self.image_:stopAllActions()
    self.sitdown_:stopAllActions()
...
function GrabDealerSeatView:isEmpty()
    return not self.seatData_
end

function GrabDealerSeatView:getPositionId()
    return self.positionId_
end

function GrabDealerSeatView:setPositionId(id)
    self.positionId_ = id
    if nk.config.GIFT_SHOP_ENABLED then
        if id then
            if id == 1 or id == 2 or id == 3 or id == 4 or id == 10 then
...
function GrabDealerSeatView:resetToEmpty()
    self.seatData_ = nil
    self:updateState()
end

function GrabDealerSeatView:setSeatData(seatData)
...
function GrabDealerSeatView:getSeatData()
    return self.seatData_
end


function GrabDealerSeatView:userGiftImageLoadCallback_(success, sprite)
    if success then
...
function GrabDealerSeatView:updateGiftUrl(gift_Id)
    if nk.config.GIFT_SHOP_ENABLED then
        if self.giftUrlReqId_ then
            LoadGiftControl:getInstance():cancel(self.giftUrlReqId_)
        end
...
function GrabDealerSeatView:updateHeadImage(imgurl)
...
function GrabDealerSeatView:updateState()
...
function GrabDealerSeatView:setHandCardValue(cards)
    if cards then
        printf("GrabDealerSeatView[%s]setHandCardValue [%x %x %x][%s %s %s][%s %s %s]", self.seatId_ or -1, cards[1] or 0, cards[2] or 0, cards[3] or 0, cards[1] or 0, cards[2] or 0, cards[3] or 0, nk.getCardDesc(cards[1]), nk.getCardDesc(cards[2]), nk.getCardDesc(cards[3]))
    end
    self.handCards_:setCards(cards)
end

function GrabDealerSeatView:setHandCardNum(num)
    self.handCards_:setCardNum(num)
end

function GrabDealerSeatView:showHandCards()
    self.handCards_:show()
end

function GrabDealerSeatView:hideHandCards()
    self.handCards_:hide()
end

function GrabDealerSeatView:showHandCardBackAll()
    self.handCards_:showBackAll()
end

function GrabDealerSeatView:showHandCardFrontAll()
    self.handCards_:showFrontAll()
end

function GrabDealerSeatView:flipAllHandCards()
    self.handCards_:flipAll()
end

function GrabDealerSeatView:hideAllHandCardsElement()
    self.handCards_:hideAllCards()
end

function GrabDealerSeatView:showAllHandCardsElement()
    self.handCards_:showAllCards()
end

function GrabDealerSeatView:showHandCardsElement(idx)
    self.handCards_:showWithIndex(idx)
end

function GrabDealerSeatView:flipHandCardsElement(idx)
    self.handCards_:flipWithIndex(idx)
end

function GrabDealerSeatView:shakeAllHandCards()
    self.handCards_:shakeWithNum(3)
end

-- 三公结束亮出自己手牌在博定中无用
function GrabDealerSeatView:showHandCardsAnimation()
...
function GrabDealerSeatView:showCardTypeIf()
...
function GrabDealerSeatView:playExpChangeAnimation(expChange)
    if expChange > 0 then
...
function GrabDealerSeatView:playAutoBuyinAnimation(buyinChips)
    local buyInBg = display.newSprite("#buyin-action-yellowbackground.png")
        :addTo(self, 6)
    local buyInBgSize = buyInBg:getContentSize()
    buyInBg:pos(0, -GrabDealerSeatView.HEIGHT/2 + buyInBgSize.height/2)
    ...
    local buyInLabel = display.newTTFLabel({
            ...
            size = 32, 
            align = ui.TEXT_ALIGN_CENTER
        }):addTo(self, 6):pos(0, -GrabDealerSeatView.HEIGHT/2 + buyInBgSize.height/2 + buyInLabelPaddding)
    ...
    local buyInLabelSequence = transition.sequence({
            spawn({
                ...
                cc.MoveTo:create(1, cc.p(0, GrabDealerSeatView.HEIGHT/2 - buyInBgSize.height/2 - buyInLabelPaddding)),
            }),
            cc.CallFunc:create(function()
...
function GrabDealerSeatView:playWinAnimation()
...
function GrabDealerSeatView:stopWinAnimation_()
    self.winBorderBatch_:hide()
    self.winnerAnimBatch_:hide()
    self.winner_:stopAllActions()
    self.star1_:stopAllActions()
...
function GrabDealerSeatView:onTouch_(target, evt)
    if evt == bm.TouchHelper.CLICK then
        nk.SoundManager:playSound(nk.SoundManager.CLICK_BUTTON)
        self:dispatchEvent({name=GrabDealerSeatView.CLICKED, seatId=self.seatId_, target=self})
    end
end

function GrabDealerSeatView:reset()
    self.handCards_:showAllCards()
    self.handCards_:showBackAll()
...
function GrabDealerSeatView:openGiftPopUpHandler()
    local roomUid = ""
...
function GrabDealerSeatView:onCleanup()
...
function GrabDealerSeatView:dispose()
    self.handCards_:dispose()
    self:release()
end

return GrabDealerSeatView