...
local GrabSeatView = class("GrabSeatView", function() 
    return display.newNode()
end)

GrabSeatView.CLICKED = "GrabSeatView.CLICKED"
GrabSeatView.WIDTH = 108
GrabSeatView.HEIGHT = 164

function GrabSeatView:ctor(ctx, seatId)
    self:retain()
    self.ctx = ctx
    cc(self):addComponent("components.behavior.EventProtocol"):exportMethods()
    self:setNodeEventEnabled(true)
    --2 桌子背景
    --self.background_ = display.newScale9Sprite("#room_seat_bg.png", 0, 0, cc.size(GrabSeatView.WIDTH, GrabSeatView.HEIGHT)):addTo(self, 2)
    ...
    if appconfig.LANG == "vn" then
        self.state_ = display.newTTFLabel({text = "Seat" .. seatId, size = 19, align = ui.TEXT_ALIGN_CENTER, color=cc.c3b(0xff, 0xd1, 0x0)})
    else
...
function GrabSeatView:playSitDownAnimation(onCompleteCallback)
...
function GrabSeatView:playStandUpAnimation(onCompleteCallback)
...
function GrabSeatView:standUpState_()
    self.image_:stopAllActions()
    self.sitdown_:stopAllActions()
...
function GrabSeatView:isEmpty()
    return not self.seatData_
end

function GrabSeatView:getPositionId()
    return self.positionId_
end

function GrabSeatView:setPositionId(id)
    self.positionId_ = id
    if nk.config.GIFT_SHOP_ENABLED then
        if id then
...
function GrabSeatView:resetToEmpty()
    self.seatData_ = nil
    self:updateState()
end

function GrabSeatView:setSeatData(seatData)
...
function GrabSeatView:getSeatData()
    return self.seatData_
end


function GrabSeatView:userGiftImageLoadCallback_(success, sprite)
    if success then
...
function GrabSeatView:updateGiftUrl(gift_Id)
    if nk.config.GIFT_SHOP_ENABLED then
        if self.giftUrlReqId_ then
            LoadGiftControl:getInstance():cancel(self.giftUrlReqId_)
        end
...
function GrabSeatView:updateHeadImage(imgurl)
...
function GrabSeatView:updateState()
...
function GrabSeatView:setHandCardValue(cards)
    if cards then
        printf("GrabSeatView[%s]setHandCardValue [%x %x %x][%s %s %s][%s %s %s]", self.seatId_ or -1, cards[1] or 0, cards[2] or 0, cards[3] or 0, cards[1] or 0, cards[2] or 0, cards[3] or 0, nk.getCardDesc(cards[1]), nk.getCardDesc(cards[2]), nk.getCardDesc(cards[3]))
    end
    self.handCards_:setCards(cards)
end

function GrabSeatView:setHandCardNum(num)
    self.handCards_:setCardNum(num)
end

function GrabSeatView:showHandCards()
    self.handCards_:show()
end

function GrabSeatView:hideHandCards()
    self.handCards_:hide()
end

function GrabSeatView:showHandCardBackAll()
    self.handCards_:showBackAll()
end

function GrabSeatView:showHandCardFrontAll()
    self.handCards_:showFrontAll()
end

function GrabSeatView:flipAllHandCards()
    self.handCards_:flipAll()
end

function GrabSeatView:hideAllHandCardsElement()
    self.handCards_:hideAllCards()
end

function GrabSeatView:showAllHandCardsElement()
    self.handCards_:showAllCards()
end

function GrabSeatView:showHandCardsElement(idx)
    self.handCards_:showWithIndex(idx)
end

function GrabSeatView:flipHandCardsElement(idx)
    self.handCards_:flipWithIndex(idx)
end

function GrabSeatView:shakeAllHandCards()
    self.handCards_:shakeWithNum(3)
end

-- 三公结束亮出自己手牌在博定中无用
function GrabSeatView:showHandCardsAnimation()
...
function GrabSeatView:showCardTypeIf()
...
function GrabSeatView:playExpChangeAnimation(expChange)
    if expChange > 0 then
...
function GrabSeatView:playAutoBuyinAnimation(buyinChips)
    local buyInBg = display.newSprite("#buyin-action-yellowbackground.png")
        :addTo(self, 6)
    local buyInBgSize = buyInBg:getContentSize()
    buyInBg:pos(0, -GrabSeatView.HEIGHT/2 + buyInBgSize.height/2)
    ...
    local buyInLabel = display.newTTFLabel({
            ...
            size = 32, 
            align = ui.TEXT_ALIGN_CENTER
        }):addTo(self, 6):pos(0, -GrabSeatView.HEIGHT/2 + buyInBgSize.height/2 + buyInLabelPaddding)
    ...
    local buyInLabelSequence = transition.sequence({
            spawn({
                ...
                cc.MoveTo:create(1, cc.p(0, GrabSeatView.HEIGHT/2 - buyInBgSize.height/2 - buyInLabelPaddding)),
            }),
            cc.CallFunc:create(function()
...
function GrabSeatView:playWinAnimation()
...
function GrabSeatView:stopWinAnimation_()
    self.winBorderBatch_:hide()
    self.winnerAnimBatch_:hide()
    self.winner_:stopAllActions()
    self.star1_:stopAllActions()
...
function GrabSeatView:onTouch_(target, evt)
    if evt == bm.TouchHelper.CLICK then
        nk.SoundManager:playSound(nk.SoundManager.CLICK_BUTTON)
        self:dispatchEvent({name=GrabSeatView.CLICKED, seatId=self.seatId_, target=self})
    end
end

function GrabSeatView:reset()
    self.handCards_:showAllCards()
    self.handCards_:showBackAll()
...
function GrabSeatView:openGiftPopUpHandler()
    local roomUid = ""
...
function GrabSeatView:onCleanup()
