...
local GrabChipManager = class("GrabChipManager")

local scheduler = require(cc.PACKAGE_NAME .. ".scheduler")
local GrabChipData = import(".model.GrabChipData")
local BetChipView = import(".views.GrabBetChipView")
local PotChipView = import(".views.GrabPotChipView")
...
local logger = bm.Logger.new("GrabChipManager")

function GrabChipManager:ctor()
end

function GrabChipManager:createNodes()
...
-- 登录成功，设置登录筹码堆
function GrabChipManager:setLoginChipStacks()
...
-- 坐下动画，移动筹码堆
function GrabChipManager:moveChipStack()
    for i = 0, 9 do
...
-- 筹码下注动画
function GrabChipManager:betChip(player)
...
function GrabChipManager:clearChip(seatId)    
    local positionId    = self.seatManager:getSeatPositionId(seatId)
...
-- 设置下注筹码数字
function GrabChipManager:modifyBetText(chips, positionId)
    if chips > 0 then
...
-- 合奖池动画
function GrabChipManager:gatherPot()
    -- 设置需要移至奖池区的筹码数据
    for i = 0, 9 do --8
        self.betChipViews_[i]:setPotGrabChipData()
    end
...
function GrabChipManager:gatherPotDelayCallback_()
    for seatId = 0, 9 do --8
...
function GrabChipManager:createPotDelayCallback_()    
    self.potChipViews_[1]:resetChipStack(self.model.gameInfo.totalAnte)
    self:modifyPotText(self.model.gameInfo.totalAnte, 1) 
end

-- 设置奖池筹码数字
function GrabChipManager:modifyPotText(chips, potId)
    -- if chips > 0 then
...
-- 分奖池
function GrabChipManager:splitPots()
...
-- 从对象池获取筹码数据
function GrabChipManager:getGrabChipData(chips)
    local numStr = tostring(chips)
    local strLen = string.len(numStr)
    local GrabChipDataArr = {}
    for i = strLen, 1, -1 do
        local oddOrEven
        local value
        ...
        if value > 5 then
            table.insert(GrabChipDataArr, self.chipPool_[oddOrEven][5]:retrive())
            value = value - 5
        end
        while value >= 2 do
            table.insert(GrabChipDataArr, self.chipPool_[oddOrEven][2]:retrive())
            value = value - 2
        end
        if value == 1 then
            table.insert(GrabChipDataArr, self.chipPool_[oddOrEven][1]:retrive())
        end
    end
    return GrabChipDataArr
end

-- 回收筹码数据
function GrabChipManager:recycleGrabChipData(GrabChipDataArr)
    if GrabChipDataArr then
        for _, GrabChipData in pairs(GrabChipDataArr) do
            GrabChipData:getSprite():opacity(255):removeFromParent()
            self.chipPool_[GrabChipData:getOddOrEven()][GrabChipData:getKey()]:recycle(GrabChipData)
        end
    end
end

-- 重置筹码视图
function GrabChipManager:reset()
    -- 重置定时器
    if self.gatherPotScheduleHandle_ then
        scheduler.unscheduleGlobal(self.gatherPotScheduleHandle_)
    end
...
-- 清理
function GrabChipManager:dispose()
    -- 重置定时器
    if self.gatherPotScheduleHandle_ then
        scheduler.unscheduleGlobal(self.gatherPotScheduleHandle_)
    end
