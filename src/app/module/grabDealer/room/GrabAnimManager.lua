...
local YouWinAnim = import(".views.GrabYouWinAnim")
local RoomViewPosition = import(".views.GrabRoomViewPosition")
local SendChipView = import(".views.GrabSendChipView")
local RoomChatBubble = import(".views.GrabRoomChatBubble")
local RoomSignalIndicator = import(".views.GrabRoomSignalIndicator")
local WinCardAnim = import(".views.GrabWinCardAnim")

local HddjController = import(".GrabHddjController")
...
local GrabAnimManager = class("GrabAnimManager")
function GrabAnimManager:ctor()
    
end

function GrabAnimManager:createNodes()
...
-- 发光
function GrabAnimManager:lightDealer_()    
...
function GrabAnimManager:onSignalStrengthChanged_(strength)
    self.signal_:setSignalStrength(strength or 5)
end

-- positionId = 10 移动到荷官位置
function GrabAnimManager:moveDealerTo(positionId, animation)   
...
function GrabAnimManager:rotateDealer(step)
...
function GrabAnimManager:playYouWinAnim()
...
function GrabAnimManager:playWinCardAnim()
    do return end
...
function GrabAnimManager:playAddFriendAnimation(fromSeatId, toSeatId)
...
function GrabAnimManager:playSendChipAnimation(fromSeatId, toSeatId, chips)
...
function GrabAnimManager:playSendGiftAnimation(giftId, fromUid, toUidArr)
...
function GrabAnimManager:checkUidInSeat(uidArr)
...
function GrabAnimManager:playHddjAnimation(fromSeatId, toSeatId, hddjId)
...
function GrabAnimManager:playExpression(seatId, expressionId)
    ...
    if self.model.playerList[seatId] then
        ...
        if anim then
            ...
            if not self.loadingExpressions_[animName] then
                ...
                display.addSpriteFrames("expressions/expression_" .. expressionId ..".plist", "expressions/expression_" .. expressionId ..".png",
                    function(fileName, imgName)
                        if self.disposed_ then
...
function GrabAnimManager:playExpressionAnim_(seatId, expressionId, anim)
    if self.model.playerList[seatId] then
        local config = ExpressionConfig:getConfig(expressionId)
...
--座位扣钱动画
function GrabAnimManager:playChipsChangeAnimation(seatId, chipsChange)
...
--显示聊天消息
function GrabAnimManager:showChatMsg(seatId, message)
...
function GrabAnimManager:removeAllAnimation()
...
function GrabAnimManager:dispose()
    if self.youWinScheduleHandle_ then
        scheduler.unscheduleGlobal(self.youWinScheduleHandle_)
    end
...
function GrabAnimManager:bindDataObservers_()
