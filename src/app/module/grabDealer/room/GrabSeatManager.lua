...
--
local GrabSeatManager = class("GrabSeatManager")

local SeatView = import(".views.GrabSeatView")
local GrabDealerSeatView = import(".views.GrabDealerSeatView")
local RoomViewPosition = import(".views.GrabRoomViewPosition")
local SeatProgressTimer = import(".views.GrabSeatProgressTimerEx")
local UserInfoOtherDialog = import(".views.GrabUserInfoOtherDialog")
local BuyInPopup = import(".views.GrabBuyInPopup")
...
local logger = bm.Logger.new("GrabSeatManager")
local USE_COUNTER_POOL = false

function GrabSeatManager:ctor(ctx)
...
function GrabSeatManager:createNodes()
    --创建座位
    self.seats_ = {}
    for i = 0, 8 do
...
function GrabSeatManager:onAppEnterBackground_()
...
function GrabSeatManager:onAppEnterForeground_()
...
function GrabSeatManager:dispose()
    if self.seats_ then
        for i = 0, 9 do
...
function GrabSeatManager:getSeatView(seatId)
    return self.seats_[seatId]
end

function GrabSeatManager:getSelfSeatView()
    return self:getSeatView(self.model:selfSeatId())
end

function GrabSeatManager:getSeatPosition(seatId)
...
function GrabSeatManager:getSeatPositionId(seatId)
    local seat = self.seats_[seatId]
    if seat then
...
function GrabSeatManager:getEmptySeatId()
...
function GrabSeatManager:initSeats(seatsInfo, playerList)
...
function GrabSeatManager:rotateSeatToOrdinal()
...
function GrabSeatManager:rotateSelfSeatToCenter(selfSeatId, animation)
    if self.dealCardRotateShowDelayId_ then
        self.schedulerPool:clear(self.dealCardRotateShowDelayId_)
        self.dealCardRotateShowDelayId_ = nil
    end
...
function GrabSeatManager:rotateSeatByStep_(step, animation)
    if step > 4 then
        step = step - 9
    elseif step < -4 then
        step = step + 9
...
function GrabSeatManager:updateAllSeatState()
    for i = 0, 9 do
        local seat = self.seats_[i]
...
function GrabSeatManager:updateSeatState(seatId)
    local seat = self.seats_[seatId]
...
function GrabSeatManager:playSitDownAnimation(seatId)
    local seat = self.seats_[seatId]
    if seat then
...
function GrabSeatManager:fadeSeat(seatId)
    local seat = self.seats_[seatId]
    if seat then
        seat:fade()
    end
...
function GrabSeatManager:playStandUpAnimation(seatId, onCompleteCallback)
    local seat = self.seats_[seatId]
    if seat then
...
function GrabSeatManager:updateHeadImage(seatId, imageUrl)
    local seat = self.seats_[seatId]
    if seat then
        if imageUrl then
...
function GrabSeatManager:updateGiftUrl(seatId, giftId)
    local seat = self.seats_[seatId]
...
function GrabSeatManager:playSeatWinAnimation(seatId)
    local seat = self.seats_[seatId]
    if seat then
...
function GrabSeatManager:stopCounter()
    for i = 0, 9 do
...
function GrabSeatManager:stopCounterOnSeat(seatId)
...
function GrabSeatManager:startCounter(seatId)  
...
function GrabSeatManager:onSeatClicked_(evt)
...
function GrabSeatManager:onBuyin_(seatId, buyinChips, isAutoBuyin)
    nk.server:seatDown(seatId, buyinChips, isAutoBuyin)    
end


function GrabSeatManager:showHandCard()
    for i = 0, 9 do
        local seat = self.seats_[i]
        local seatData = seat:getSeatData()       
...
function GrabSeatManager:showHandCardByOther(seatId)   
    local seat = self.seats_[seatId]
    local seatData = seat:getSeatData()
...
function GrabSeatManager:prepareDealCards()
...
function GrabSeatManager:onAddExp(evt)
    local selfSeatId = self.model:selfSeatId()
...
--播放座位加经验的动画，只播放自己的
function GrabSeatManager:playExpChangeAnimation()
...
--刷新庄家位置(补币)
function GrabSeatManager:addDealerHandCoin(totalmoney)
...
function GrabSeatManager:reset()
    for i = 0, 9 do
        local seat = self.seats_[i]
        seat:reset()
    end
