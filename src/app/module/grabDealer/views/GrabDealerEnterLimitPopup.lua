local GrabDealerEnterLimitPopup = class("GrabDealerEnterLimitPopup", nk.ui.Panel)

local WIDTH = 598
local HEIGHT = 350

local StorePopup = import("app.module.store.StorePopup")
function GrabDealerEnterLimitPopup:ctor(param)
...
function GrabDealerEnterLimitPopup:onButtonClick_(evt)
	self:hidePanel_()
	--self.param_.enterMatch()
	StorePopup.new(2):showPanel()

end
function GrabDealerEnterLimitPopup:enterGame(evt)
	self:hidePanel_()
	self.param_.enterGame()
end
function GrabDealerEnterLimitPopup:show()
	-- body
	self:showPanel_()
end

function GrabDealerEnterLimitPopup:onShowed()
	-- body
	
end

return GrabDealerEnterLimitPopup