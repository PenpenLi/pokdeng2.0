...
-- Description: HallScene.lua Reconstructed By Tsing7x.
--
...
local AttendDokbView = import("app.module.dokbAct.AttendDokbView")
local ChooseMatchRoomView = import("app.module.hall.ChooseMatchRoomView")
...
function HallScene:ctor(viewType, action)
    ...
    self.mainHallBg_ = display.newSprite("main_hall_bg.png")
        :scale(self.bgScale_)
                ...
                if not nk.PopupManager:removeTopPopupIf() then
                    ...
                    if currentHallView == HallController.MAIN_HALL_VIEW then
                        device.showAlert(bm.LangUtil.getText("COMMON", "LOGOUT_DIALOG_TITLE"),
                            bm.LangUtil.getText("COMMON", "LOGOUT_DIALOG_MSG"), {bm.LangUtil.getText("COMMON", "CANCEL"),
                                bm.LangUtil.getText("COMMON", "LOGOUT")}, function(event)
                    ...
                    elseif currentHallView == HallController.CHOOSE_MATCH_PRO_VIEW then
                        ...
                        device.showAlert(bm.LangUtil.getText("COMMON", "QUIT_DIALOG_TITLE"),
                            bm.LangUtil.getText("COMMON", "QUIT_DIALOG_MSG"), {bm.LangUtil.getText("COMMON", "QUIT_DIALOG_CONFIRM"), 
                                bm.LangUtil.getText("COMMON", "QUIT_DIALOG_CANCEL")}, function(event)
                                if event.buttonIndex == 1 then
                                    local isAdSceneOpen = nk.OnOff:check("unionAd")
                                    
                                    if isAdSceneOpen and nk.AdSceneSdk then
                                        local state = nk.AdSceneSdk:showInterstitialAdDialog(true, function(state)
...
function HallScene:onLogoutSucc()
    ...
    if self.attendDokbView_ and self.attendDokbView_.hidePanel then
        --todo
        self.attendDokbView_:hidePanel()
        self.attendDokbView_ = nil
    end
...
function HallScene:showLoginView_()
    ...
    if not self.loginView_ then
        display.addSpriteFrames("login_th.plist", "login_th.png", function()
            -- body
            self.loginView_ = LoginGameView.new(self.controller_)
                :pos(display.cx, display.cy)
                :addTo(self, LOGIN_GAME_ZORDER)
...
function HallScene:showDokbActPageView_()
    -- body
    -- self.pokerGirlBatchNode_:scaleTo(self.animTime_, self.bgScale_ * 0.8)
    self.attendDokbView_ = AttendDokbView.new(self.controller_):showPanel()
    self.attendDokbView_:playShowAnim()
end
