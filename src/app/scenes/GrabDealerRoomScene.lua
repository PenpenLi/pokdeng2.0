...
--
local RoomController = import("app.module.grabDealer.room.GrabRoomController")
local RoomViewPosition = import("app.module.grabDealer.room.views.GrabRoomViewPosition")
local RoomImageButton = import("app.module.grabDealer.room.views.GrabRoomImageButton")
local RoomMenuPopup = import("app.module.grabDealer.room.views.GrabRoomMenuPopup")
...
local RoomChatBubble = import("app.module.grabDealer.room.views.GrabRoomChatBubble")
...
local QuickShortBillPay = import("app.module.store.quickSortPay.QuickShortBillPay")
local SeatInviteView = import("app.module.grabDealer.room.views.GrabSeatInviteView")
local GrabDealerBuyInPopup = import("app.module.grabDealer.room.views.GrabDealerBuyInPopup")
local GrabAddDealerAnteMoneyPopup = import("app.module.grabDealer.room.views.GrabAddDealerAnteMoneyPopup")
local CardTypeGameRecPopup = import("app.module.crdTypeGmRecs.CardTypeGameRecPopup")
local NormalRoomBaseView = import("app.module.grabDealer.room.views.GrabNormalRoomBaseView")
local PersonalRoomBaseView = import("app.module.grabDealer.room.views.GrabPersonalRoomBaseView")
local InvitePlayPopup = import("app.module.invitePlay.InvitePlayPopup")

local PswEntryPopup = import("app.module.hall.views.PswEntryPopup")
local GrabDealerLeftTimeTip = import("app.module.grabDealer.room.views.GrabDealerLeftTimeTip")
-- local GrabDealerAdvanceNotice = import("app.module.grabDealer.room.views.GrabDealerAdvanceNotice")
local GrabDealerPaoPaoTip = import("app.module.grabDealer.room.views.GrabDealerPaoPaoTip")
local GrabDealerRoomScene = class("GrabDealerRoomScene", function()
    return display.newScene("GrabDealerRoomScene")
end)
local logger = bm.Logger.new("GrabDealerRoomScene")

local TOP_BUTTOM_WIDTH   = 78
local TOP_BUTTOM_HEIGHT  = 58

GrabDealerRoomScene.EVT_BACKGROUND_CLICK = "EVT_BACKGROUND_CLICK"

function GrabDealerRoomScene:ctor()
...
--设置抢庄倒计时
function GrabDealerRoomScene:setGrabLeftTime(leftTime)
    if leftTime == 0 then
        self.grabLeftTimeTips_:hide()
    else
        local isCashRoom = 0
...
--设置抢庄预告
function GrabDealerRoomScene:setGrabPlayerData(data)
...
--更新抢庄预告局数
function GrabDealerRoomScene:updataGrabAdvance()
    -- self.grabAdvanceNotice_:update()
end
--庄家补币预告
function GrabDealerRoomScene:dealerAddCoin(data)
...
--设置抢庄按钮显示
function GrabDealerRoomScene:showGrabDealerBtn(pack)
...
--抢庄预告停止(新庄家坐下)
function GrabDealerRoomScene:setGrabAdvanceStop()
    -- self.grabAdvanceNotice_:stop()
end
--设置庄家补币按钮显示
function GrabDealerRoomScene:showDealerAddCoinBtn(isShow)
    if isShow == true then
...
function GrabDealerRoomScene:toDelearSendChipHandler()
...
function GrabDealerRoomScene:setStoreDiscount(discount)
...
function GrabDealerRoomScene:setRoomInfoText(roomInfo)
...
function GrabDealerRoomScene:onGrabInviteClick_()
...
function GrabDealerRoomScene:setMatchInviteButtonVisible(visible)
...
function GrabDealerRoomScene:setSlotBlind(roomInfo)
    -- local isCashRoom = 0
    -- if self.ctx.model.roomInfo.rmType == consts.ROOM_TYPE_EX.GRAB_CASH then
    --    isCashRoom = 1
    -- end
    ...
    -- if nk.config.SLOT_ENABLED and isSlotOpen and isCashRoom == 0 then
    ...
    dump("No Slot in GrabRoom Nor CashRoom!")
...
function GrabDealerRoomScene:setChangeRoomButtonMode(mode)
...
function GrabDealerRoomScene:onStandupClick_()
...
function GrabDealerRoomScene:doBackToHall(msg, jumpType)
...
function GrabDealerRoomScene:doBackToLogin(msg)
    msg = msg or bm.LangUtil.getText("ROOM", "OUT_MSG")
    if self.roomLoading_ then 
        self.roomLoading_:removeFromParent()
        self.roomLoading_ = nil
...
function GrabDealerRoomScene:doBackToLoginByDoubleLogin(msg)
    msg = msg or bm.LangUtil.getText("ROOM", "OUT_MSG")
    if self.roomLoading_ then 
        self.roomLoading_:removeFromParent()
        self.roomLoading_ = nil
...
function GrabDealerRoomScene:onMenuClick_()
    self.menuPop_ = RoomMenuPopup.new(function(tag)
        ...
        elseif tag == 3 then
            --弹出设置菜单
            SettingHelpPopup.new(true):showPanel()
        elseif tag == 4 then
...
function GrabDealerRoomScene:onChangeRoomClick_()
...
-- Abandon --
function GrabDealerRoomScene:playNowChangeRoom()
    -- self.menuPop_.callBack_(2)
    -- self:onChangeRoom_(false, true)
end

function GrabDealerRoomScene:removeLoading()
    if self.roomLoading_ then 
        self.roomLoading_:removeFromParent()
        self.roomLoading_ = nil
    end
...
function GrabDealerRoomScene:onChangeRoom_(doNotUpdateRoomInfo, isPlayNow)
...
function GrabDealerRoomScene:onSearchPersonalRoom(data)
    local ret = data.ret
...
function GrabDealerRoomScene:onShopClick_(tag)
    ...
    if self.ctx.model.roomInfo.rmType == consts.ROOM_TYPE_EX.GRAB_CASH then
        ...
        if tag == nil and isShowPay and nk.OnOff:check("quickShortPay") then
            ...
            if self.ctx.model.roomInfo.blind and getBillIdxByRoomAnte[self.ctx.model.roomInfo.blind] then
                --todo
                QuickShortBillPay.new(true, getBillIdxByRoomAnte[self.ctx.model.roomInfo.blind])
            else
                StorePopup.new(2):showPanel()
            end
        else
...
-- 首冲 --
function GrabDealerRoomScene:onChargeFavClick_(tag)
    -- body
...
-- 慈善礼包 --
function GrabDealerRoomScene:onAlmRechargeFavClick_()
    -- body
    AgnChargePayGuidePopup.new():showPanel(nil, nil, true)
end

function GrabDealerRoomScene:onCardTypeClick_()
    ...
    CardTypeGameRecPopup.new():showPanel()
end

function GrabDealerRoomScene:onInviteBtnClick_( )
    InvitePlayPopup.new():show()
end

function GrabDealerRoomScene:onLoadedHallTexture_(fileName, imgName)
...
function GrabDealerRoomScene:onLoadedHallTextureLogout_(fileName, imgName)
...
function GrabDealerRoomScene:onLoadedHallTextureDoubleLogin_()
    app:enterHallScene({HallController.LOGIN_GAME_VIEW, "doublelogin"})
end

function GrabDealerRoomScene:playDealerBubble(evt)
...
function GrabDealerRoomScene:showBubble(textId, DEALER_SPEEK_ARRAY)
...
function GrabDealerRoomScene:createNodes_()
    self.nodes = {}
...
function GrabDealerRoomScene:onEnter()
    ...
    if device.platform == "android" or device.platform == "ios" then
        cc.analytics:doCommand{command = "beginScene",
                    args = {sceneName = "GrabDealerRoomScene"}}
    end
end

function GrabDealerRoomScene:onExit()
    --清空房间聊天记录
    bm.DataProxy:clearData(nk.dataKeys.ROOM_CHAT_HISTORY)
    if device.platform == "android" or device.platform == "ios" then
    ...
    bm.DataProxy:setData(nk.dataKeys.LAST_ENTER_SCENE,"GrabDealerRoomScene")
end

function GrabDealerRoomScene:onCleanup()
    -- 控制器清理
...
function GrabDealerRoomScene:onBackgroundTouch_(target, evt)
    if evt == bm.TouchHelper.CLICK then
        self:dispatchEvent({name=GrabDealerRoomScene.EVT_BACKGROUND_CLICK})
    end
end

function GrabDealerRoomScene:doubleLoginOut_(evt)
    nk.socket.RoomSocket:sendLogout()
    self:doBackToLoginByDoubleLogin()
end

return GrabDealerRoomScene