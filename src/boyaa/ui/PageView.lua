...
function PageView:ctor(params, itemClass, pageParams)
    ...
    if pageParams ~= nil then
        ...
        self.btnNormal_ = pageParams.btnNormal or "#common_page_btn_normal.png"
        self.btnSelect_ = pageParams.btnSelect or "#common_page_btn_select.png"
    end
...
function PageView:updateData(index,data)
    local count = self.params.rows*self.params.columns
    local page = math.ceil((index/count))
    local modDesIdx = index % count
    local desIndex =  (modDesIdx == 0 and count or modDesIdx)
    if self.items_ and self.items_[page] then
        self.items_[page]:updateData(desIndex,data)
    end

end

function PageView:updateBtns(index)
    if (not self.isPage_) or (not self.itemNum_) or (not self.showIndex_) then
        return
    end
